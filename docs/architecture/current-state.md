...
| lynx_vnpy | **0.0** | 纯观察者, 不参与评分/分歧检测/仓位裁决 |
...
| 变更 | 描述 | 涉及文件 |
|------|------|---------|
| LY flat zone 移除 | 基于 708 笔 walkforward 数据遍历 15 种 flat zone, 确认无 flat zone 最优(51.5%) | `normalizer.py`, `lynx_signal.py` |
| LY L7 映射两套统一 | `normalizer.py` 与 `lynx_signal.py` 锚点表对齐为单点中性 @45 | 同上 |
| LY L7 映射后 OOS 指标 | walkforward 加算 L7 方向, c1test 解析并展示 | `lynx_signal.py`, `c1test.py` |
| op_advice 恢复 0.2 权重 | 表态率 23%→50%后回归 L7 裁决, linear+bayesian 双路径 | `fusion_engine.py` |
| 逐笔预测保存 | `--save-predictions` 参数, 支持离线 flat zone 分析 | `lynx_signal.py` |
| flat zone 搜索脚本 | `find_optimal_ly_flat.py` | 新增脚本 |
...
| 薄弱环节 | 状态 | 最新进展 |
|---------|:----:|---------|
| ML 评测口径 | **✅ 已统一** | op_advice vs sentiment 同 T+1/同 flat zone/同样本 |
| v4.0精度校准映射 | **✅ 已部署** | 7值非对称映射, normalizer.py |
| LY flat zone | **✅ 已移除** | 两套映射统一为单点中性 @45, L7 准确率 44.5%→51.3% |
| LY L7 映射后 OOS | **✅ 已新增** | 686 样本, 融合等效准确率 |
| op_advice 回归融合 | **✅ 已恢复 0.2 权重** | 表态率提升后观察融合效果 |
| 分歧惩罚→ML少数方增强 | **✅ 已部署** | 分歧时ML自适应提分+0~0.3 |
| DT数据湖整合 | **✅ 已完成** | 估值数据湖兜底 + 盘前刷新 10:50/13:50 |
| AT权重归零 | **⏳ 等待数据积累** | 新 prompt 准确率 54.8%(62样本), 待 300+ 样本后重估 |
| 融合基线56.5% | **⏳ 持续跟踪** | 最新 56.5% (269样本) |
| LY 信号天花板 | **✅ 已确认** | 经典因子在 A 股个股上信号弱, 维持现状 |

### 当前待办

| 优先级 | 任务 | 说明 |
|:------:|------|------|
| P1 | AT 数据积累与重评估 | 新 prompt 运行 2 周, 积累 ~300 样本后重新评估权重 |
| P2 | backtest --force 重算 | 用52/49阈值+HP3双路径重新评估全部历史记录 |
| P3 | op_advice 回归效果观察 | 几天后跑 c1test 对比融合准确率变化 |
...
实时行情/估值数据的获取模式原为"LLM 分析时实时调用 API"，
存在超时导致数据缺失的问题。
...
`data-warehouse.service`（RefreshScheduler）定时刷新基本面数据：

| 时间 | 触发 | 覆盖 |
|:----:|:----|:----:|
| **周一 09:00** | 周度基本面刷新 | 全量股票 |
| **每日 10:50** | 盘前预热（配合 11:00 整点分析） | 全量股票 |
| **每日 13:50** | 盘前预热（配合 14:00 整点分析） | 全量股票 |

ML 整点分析读取数据湖，**零 API 调用**。刷新失败时使用前次缓存数据（日频数据隔夜仍有效）。

### 11.4 四层数据获取链路

`get_fundamental_context()` 的优先级（`base.py`）：

```
Step 1: WarehouseReader → 数据湖命中 ✅ (0ms)
Step 2: _fundamental_cache → 内存缓存, 120s TTL ✅ (0ms)
Step 3: API → Tushare+Tencent, 20s 超时 ✅ (~5s, 写入湖+缓存)
Step 4: 数据湖旧数据 → API 失败时用前次缓存 ✅ (0ms, stale>None)
```
...
| 源 | 优先级 | 用途 | 稳定性 |
|----|:----:|------|--------|
| **数据湖** | 最高 | PE/PB/筹码/资金流 | 调度器定时填充, 过期不阻塞 |
| Tushare Pro | 高 | 实时行情/OHLCV/资金流/估值 | 稳定(已付费) |
| Tencent (akshare) | 中 | PE/PB/量比/换手率补充 | 稳定 |
| akshare (EM/Sina) | 后备 | 补充数据 | 版本依赖敏感 |
| pytdx (TCP) | 后备 | 日K线降级 | 稳定 |
