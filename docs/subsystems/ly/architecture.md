...
> 最后更新: 2026-07-06 (含 flat zone 移除 + L7 映射统一 + 融合等效指标)
...
> **2026-07-06**: flat zone 移除后 L7 映射损耗消除。当前 L7 映射后准确率 (51.3%) 已超过 raw prob_up (49.7%)。
...
**反方："Alpha158 设计目标是横截面选股，改成横截面用法会不会更好？"**

回应：2026-07-03 用 12 只股票、176 个公共交易日做了横截面 IC 验证原型(`scripts/research_alpha158_cross_section.py`)，结果：

```
横截面平均 |IC|: 0.0158  (26/59 因子为正)
时序平均 |IC|:   0.0321  (27/59 因子为正)
结论: ❌ 横截面 IC 未优于时序 IC
```

横截面用法不但没有更好，反而更差。**问题不是"怎样用"，是"Alpha158 的 58 个因子对这 12 只股票本身的信号就很弱"**——无论时序还是横截面，平均 |IC| 都远低于实用阈值 0.05。

这是一个关键发现：LY 的低预测力不是方法论错误，是**信号源的天花板**。经典量价因子在这 12 只 A 股个股上能提取的信息就这么多。
...
| 方向 | 预期提升 | 工作量 | 决策 |
|------|:--------:|:------:|:----:|
| 改用横截面选股范式 (panel data) | ❌ 实测无效 | 大 | **不实施** |
| 增加另类数据（资金流/情绪/新闻因子） | 中 | 中 | 待评估 |
| prob_up 校准（概率输出校准） | 小~中 | 小 | 待评估 |
| 放弃 RF，仅保留 LGB | 已完成 | - | ✅ 已实施 |
| **当前策略：维持权重 0.37，融合贡献稳定** | — | 0 | ✅ 执行中 |
