...
| **层2: 16 策略** | ❌ **不可独立** | 策略是 LLM 驱动的 YAML 文本，不是代码 | N/A |
...
| **5** | `turnover_sentiment` | 换手率情绪 | 情绪 | 近5日均换手率/全历史均换手率 | 值越大→异常放量→筹码分散风险 | 0.050 | 0.08 | **7.0%** |
...
16 个 YAML 策略文件定义在 `systems/MindLynx-Aistock/strategies/` 下，
...
**按类别分布**：trend 5个 / reversal 1个 / framework 8个 / pattern 1个 / **risk_control 1个 🆕**
**理论来源**：道氏、威科夫、葛兰碧、欧奈尔、勒庞、索罗斯、马克斯、林奇、达瓦斯、缠中说禅、艾略特、尼森、**格雷厄姆** — 13个理论。
