...
`should_inject = True` 后，AT Agent 同时拥有注入数据 + 自身 yfinance 工具结果。LLM 处理数据冲突行为不稳定：可能忽略注入、可能混淆两份数据、可能给中庸结论。创使状态（create_initial_state）中注入的 AIMessage 排在消息列表开头，后续工具结果会覆盖其注意力。

**4. LY→ML 注入模式不能直接类比 AT**

| 维度 | LY→ML | LY→AT |
|------|-------|-------|
| LLM 数量 | 单一分析器 | **10 个 Agent 辩论** |
| 数据冲突 | 无（ML 不另取行情） | **有**（同时用 yfinance + 注入） |
| 信号位置 | Prompt 显式段落 | create_initial_state → 被后续工具结果覆盖 |
| 输出格式 | 结构化 JSON | 5 级评级 + 自由辩论文本 |

#### 假设检验

| 假设 | 报告立场 | 反方证据 | 判定 |
|------|---------|---------|------|
| "AT 有统计显著的预测能力" | 隐含（方案 A/B/C 都假设 AT 值得救） | 26/48 p≈0.67，无法拒绝随机 | ❌ 不成立 |
| "注入客观信号能改善 LLM 辩论" | 是 | LLM 处理数据冲突不稳定，注入位置易被覆盖 | ⚠️ 未验证 |
| "方案 B 实施周期 < 1 周" | 是 | 代码 150 行，验证需 **1 个月**（20+交易日） | ⚠️ 低估 |
...
| **P1** | 如果 P0 通过 → **方案 A**（仅 LY 注入，保持 fast_degrade） | ~80 行 | P0 证明 AT 有信号 |
| **P2** | 如果 P1 有效 → 扩展 at_signal.json 字段（加置信度/数据源） | ~60 行 | P1 回测提升 > 3% |
| **P3** | 方案 B/C + 相关性监控（AT vs LY/ML） | ~180 行 | P1+P2 验证通过 |
...
报告的**问题诊断质量很高**（数据源不可用、缺少客观锚定），但**改进方案存在「诊断正确但开错药方」的风险**——把 AT 的 54.2% 归因于"缺少数据"而非"方法本身局限性"，可能导致投入 150+ 行代码和 1 个月验证后效果甚微。

**最安全的路径**: 先做 P0（统计检验 + 降权），根据结果决定是否继续。在 AT 被验证有统计显著信号之前，方案 A/B/C 的实施应暂缓。
