...
def extract_symbol_from_ts_code(ts_code: str, market: str) -> str | None:
    """
...
def get_stock_name(row: dict[str, str], market: str) -> str | None:
    """
    获取股票名称
...
def parse_stock_row(row: dict[str, str], preferred_market: str | None = None) -> dict[str, Any] | None:
    """
    解析单行股票数据
...
def main():
    ...
    parser.add_argument(
        "--source", choices=["tushare", "akshare"], default="tushare", help="数据源选择（默认: tushare）"
    )
    parser.add_argument("--test", "-t", action="store_true", help="测试模式：只验证不写入文件")
