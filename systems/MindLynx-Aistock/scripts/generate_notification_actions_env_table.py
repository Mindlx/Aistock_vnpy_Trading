...
def extract_managed_block(markdown: str) -> str:
    ...
    if start == -1 or end == -1 or end < start:
        raise ValueError(f"Could not find managed table markers {TABLE_START!r} and {TABLE_END!r}")
...
def validate_required_mappings(env: dict[str, str]) -> None:
    ...
    if missing:
        raise ValueError("daily_analysis.yml is missing required notification env mappings: " + ", ".join(missing))
