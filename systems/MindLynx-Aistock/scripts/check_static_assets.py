...
def main(argv: list[str]) -> int:
    ...
    if missing:
        print(
            "[check_static_assets] ERROR: index.html references assets that are not present on disk:",
            file=sys.stderr,
        )
        for ref in missing:
    ...
    print(f"[check_static_assets] OK: {len(referenced)} asset reference(s) resolved successfully.")
    return 0
