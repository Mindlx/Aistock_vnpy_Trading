...
def get_tushare_api() -> ts.pro_api | None:
    """
    获取 Tushare API 实例
...
def fetch_a_stock_list(api: ts.pro_api) -> pd.DataFrame | None:
    """
    获取 A股列表
...
def fetch_hk_stock_list(api: ts.pro_api) -> pd.DataFrame | None:
    """
    获取港股列表
    ...
    try:
        # 获取所有正常上市的港股
        df = api.hk_basic(list_status="L")  # L: 上市, D: 退市
...
def fetch_us_stock_list(api: ts.pro_api) -> pd.DataFrame | None:
    """
    获取美股列表（分页读取）
...
def generate_data_documentation(a_df: pd.DataFrame | None, hk_df: pd.DataFrame | None, us_df: pd.DataFrame | None):
    """
    生成数据说明文档
