...
def _get_with_retry(
    url: str, *, headers: dict[str, str], params: dict[str, Any] | None = None, timeout: int = _REQUEST_TIMEOUT
) -> requests.Response:
    ...
    def _format_social_intel(
        ...
        if reddit_data:
            ...
            if buzz is not None:
                ...
                lines.append(
                    f"  Buzz Score: {buzz}/100 ({trend_label})" if trend_label else f"  Buzz Score: {buzz}/100"
                )
        ...
        if x_entry:
            ...
            if x_buzz is not None:
                lines.append(f"  Buzz Score: {x_buzz}/100 ({x_trend})" if x_trend else f"  Buzz Score: {x_buzz}/100")
