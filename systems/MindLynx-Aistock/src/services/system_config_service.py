...
class SystemConfigService:
    ...
    def _collect_setup_channel_models(cls, effective_map: dict[str, str]) -> list[str]:
        ...
        for raw_name in cls._split_csv(effective_map.get("LLM_CHANNELS") or ""):
            ...
            if name.lower() == "anspire" and not base_url:
                base_url = (effective_map.get("ANSPIRE_LLM_BASE_URL") or ANSPIRE_LLM_BASE_URL_DEFAULT).strip()
    ...
    def _infer_setup_legacy_primary_model(cls, effective_map: dict[str, str]) -> str:
        ...
        if cls._anspire_legacy_llm_enabled(effective_map) and cls._has_any_config_value(
            ...
            model = (
                effective_map.get("ANSPIRE_LLM_MODEL") or effective_map.get("OPENAI_MODEL") or ANSPIRE_LLM_MODEL_DEFAULT
            ).strip()
            return model if "/" in model else f"openai/{model}"
    ...
    def _build_setup_agent_llm_check(
        ...
        configured_models = set(
            self._collect_yaml_models_from_map(effective_map) or self._collect_setup_channel_models(effective_map)
        )
        ...
        if (
            not configured_models and self._has_setup_runtime_source_for_model(agent_model, effective_map)
    ...
    def _build_setup_notification_check(self, effective_map: dict[str, str]) -> dict[str, Any]:
        configured = (
            self._has_any_config_value(
                effective_map, ("WECHAT_WEBHOOK_URL", "FEISHU_WEBHOOK_URL", "DISCORD_WEBHOOK_URL")
            )
            or (
    ...
    @classmethod
    def _sanitize_llm_details(cls, details: dict[str, Any] | None) -> dict[str, Any]:
        if not details:
            return {}
        ...
        for key, value in details.items():
            if isinstance(value, str):
                ...
                sanitized[key] = [
                    cls._sanitize_llm_error_text(item) if isinstance(item, str) else item for item in value
                ]
            else:
                sanitized[key] = value
        return sanitized
    ...
    def _classify_llm_http_error(status_code: int, error_text: str) -> _LLMDiagnostic:
        ...
        if status_code in {401, 403} or any(
            token in lowered for token in ("unauthorized", "forbidden", "invalid api key", "authentication")
        ):
    ...
    def _classify_llm_exception(exc: Exception) -> _LLMDiagnostic:
        ...
        if any(token in exc_name for token in ("auth", "permission")) or any(
            token in text for token in ("unauthorized", "forbidden", "invalid api key", "authentication")
        ):
            return _LLMDiagnostic("auth", False, "LLM authentication failed", "api_key_rejected")
        if (
            ("notfound" in exc_name or "model" in text)
            and ("not found" in text or "does not exist" in text or "unknown model" in text)
        ...
        if "dns" in text or "name resolution" in text or "temporary failure in name resolution" in text:
            return _LLMDiagnostic(
                "network_error", True, "LLM request failed before a valid response was returned", "dns_error"
            )
        if "refused" in text or "connection refused" in text:
            return _LLMDiagnostic(
                "network_error", True, "LLM request failed before a valid response was returned", "connection_refused"
            )
        if "ssl" in text or "tls" in text or "certificate" in text:
            return _LLMDiagnostic(
                "network_error", True, "LLM request failed before a valid response was returned", "tls_error"
            )
        if any(token in exc_name for token in ("connection", "network")) or any(
            token in text for token in ("connection", "network", "firewall")
        ):
            return _LLMDiagnostic(
                "network_error", True, "LLM request failed before a valid response was returned", "network_error"
            )
    ...
    def _validate_llm_channel_map(effective_map: dict[str, str], updated_keys: set[str]) -> list[dict[str, Any]]:
        ...
        for name in normalized_names:
            ...
            if name.lower() == "anspire" and not base_url_value:
                base_url_value = (effective_map.get("ANSPIRE_LLM_BASE_URL") or ANSPIRE_LLM_BASE_URL_DEFAULT).strip()
            ...
            models_value = [
                model.strip() for model in (effective_map.get(f"{prefix}_MODELS") or "").split(",") if model.strip()
            ]
            if name.lower() == "anspire" and not models_value:
                models_value = [(effective_map.get("ANSPIRE_LLM_MODEL") or ANSPIRE_LLM_MODEL_DEFAULT).strip()]
            enabled_raw = effective_map.get(f"{prefix}_ENABLED")
            if name.lower() == "anspire" and not (enabled_raw or "").strip():
                enabled_raw = effective_map.get("ANSPIRE_LLM_ENABLED")
            enabled = parse_env_bool(enabled_raw, default=True)
    ...
    def _collect_llm_channel_models_from_map(effective_map: dict[str, str]) -> list[str]:
        ...
        for raw_name in raw_channels.split(","):
            ...
            if name.lower() == "anspire" and not raw_models:
                raw_models = [(effective_map.get("ANSPIRE_LLM_MODEL") or ANSPIRE_LLM_MODEL_DEFAULT).strip()]
            resolved_protocol = resolve_llm_channel_protocol(
                protocol_value, base_url=base_url_value, models=raw_models, channel_name=name
            )
    ...
    def _validate_llm_runtime_selection(effective_map: dict[str, str]) -> list[dict[str, Any]]:
                ...
                issues.append(
                    {
                        ...
                        "code": "missing_runtime_source",
                        "message": (
                            "Some fallback models do not have an enabled channel or matching API key available"
                        ),
                        "severity": "error",
        ...
        fallback_models = [
            model.strip() for model in (effective_map.get("LITELLM_FALLBACK_MODELS") or "").split(",") if model.strip()
        ]
        invalid_fallbacks = [
            model
            for model in fallback_models
