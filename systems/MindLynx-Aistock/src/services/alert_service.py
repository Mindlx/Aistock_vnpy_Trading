...
class AlertService:
    ...
    def _normalize_rule_payload(self, payload: dict[str, Any], *, source: str = "api") -> dict[str, Any]:
        ...
        if alert_type not in SUPPORTED_ALERT_TYPES:
            raise UnsupportedAlertTypeError(f"unsupported alert_type for P1 Alert API: {alert_type or '<empty>'}")
    ...
    def _serialize_cooldown_summary(row: AlertCooldownRecord | None) -> dict[str, Any]:
        ...
        cooldown_active = bool(
            row.state == "active" and row.cooldown_until is not None and row.cooldown_until > datetime.now()
        )
        return {
