...
KEY_SPECS: tuple[NotificationKeySpec, ...] = (
    tuple(
        NotificationKeySpec(
            key=key, tier="minimal", description="Required to enable the channel.", channel=spec.channel
        )
        for spec in CHANNEL_SPECS
        for key in (spec.minimal_keys + tuple(key for key_group in spec.alternative_minimal_keys for key in key_group))
    )
    + tuple(
        NotificationKeySpec(
            key=key, tier="advanced", description="Optional channel behavior or security setting.", channel=spec.channel
        )
        for spec in CHANNEL_SPECS
...
P3_ROUTE_ENV_KEYS: tuple[str, ...] = tuple(route["env_key"] for route in NOTIFICATION_ROUTE_CONFIGS.values())
...
def run_notification_diagnostics(config: Config) -> NotificationDiagnosticResult:
    ...
    if (_has(config, "feishu_webhook_secret") or _has(config, "feishu_webhook_keyword")) and not _has(
    ...
    if (_has(config, "custom_webhook_bearer_token") or _has(config, "custom_webhook_body_template")) and not _has(
    ...
    if getattr(config, "notification_daily_digest_enabled", False):
        warnings.append(
            _issue(
                ...
                ("NOTIFICATION_DAILY_DIGEST_ENABLED 当前为预留配置；P4 不会发送每日摘要或持久化摘要内容。"),
...
def format_notification_diagnostics(result: NotificationDiagnosticResult) -> str:
    ...
    if result.configured_channels:
        channel_names = [
            ChannelDetector.get_channel_name(NotificationChannel(channel)) for channel in result.configured_channels
        ]
