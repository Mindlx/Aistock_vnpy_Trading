...
def _canonicalize_lookup_value(value: Any, canonical_map: dict[str, str]) -> str | None:
    for candidate in _iter_lookup_candidates(value):
        ...
        if canonical:
            return canonical
    # Fallback: check if any canonical keyword exists as a non-negated substring
    raw_text = str(value or "").strip()
    if raw_text:
        for keyword, canonical_value in canonical_map.items():
            if not keyword:
                continue
            keyword_norm = _normalize_lookup_key(keyword)
            if not keyword_norm or keyword_norm not in raw_text:
                continue
            # Reuse negation detection: skip if all keyword occurrences are negated
            if _first_non_negated_position(raw_text, keyword_norm) is not None:
