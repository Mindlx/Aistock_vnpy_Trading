...
class PortfolioRepository:
    ...
    def get_account_in_session(
        ...
        *,
        ...
        return session.execute(select(PortfolioAccount).where(and_(*conditions)).limit(1)).scalar_one_or_none()
    ...
    def delete_trade_in_session(self, *, session: Any, trade_id: int) -> bool:
        row = session.execute(select(PortfolioTrade).where(PortfolioTrade.id == trade_id).limit(1)).scalar_one_or_none()
        if row is None:
            return False
        self._invalidate_account_cache_in_session(
            session=session,
    ...
    def query_trades(
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            rows = (
                session.execute(
                    data_query.order_by(PortfolioTrade.trade_date.desc(), PortfolioTrade.id.desc())
    ...
    def query_cash_ledger(
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            rows = (
                session.execute(
                    data_query.order_by(PortfolioCashLedger.event_date.desc(), PortfolioCashLedger.id.desc())
                    .offset((page - 1) * page_size)
                    .limit(page_size)
                )
                .scalars()
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            rows = (
                session.execute(
                    data_query.order_by(
                        PortfolioCorporateAction.effective_date.desc(), PortfolioCorporateAction.id.desc()
                    )
                    .offset((page - 1) * page_size)
                    .limit(page_size)
                )
    ...
    def _translate_trade_integrity_error(
        *,
        ...
        if trade_uid and ("uix_portfolio_trade_uid" in err_text or "unique" in err_text):
            return DuplicateTradeUidError(f"Duplicate trade_uid for account_id={account_id}: {trade_uid}")
        if dedup_hash and (
            ...
            return DuplicateTradeDedupHashError(f"Duplicate dedup_hash for account_id={account_id}: {dedup_hash}")
        return exc
