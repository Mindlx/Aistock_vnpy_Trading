...
class BacktestRepository:
    ...
    def get_results_paginated(
        ...
        *,
        ...
        offset: int,
        limit: int,
    ) -> tuple[list[tuple[BacktestResult, str | None, str | None, datetime | None]], int]:
        with self.db.get_session() as session:
    ...
    def count_results(
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            count = (
                session.execute(
                    select(func.count(BacktestResult.id)).select_from(BacktestResult).where(where_clause)
                ).scalar()
                or 0
            )
            return int(count)
    ...
    def upsert_summary(self, summary: BacktestSummary) -> None:
        ...
        with self.db.get_session() as session:
            ...
            if summary.skill_id is not None:
                conditions.append(BacktestSummary.skill_id == summary.skill_id)
            else:
                conditions.append(BacktestSummary.skill_id.is_(None))

            existing = session.execute(
                select(BacktestSummary).where(and_(*conditions)).limit(1)
            ).scalar_one_or_none()
        ...
        *,
        ...
        with self.db.get_session() as session:
            ...
            if skill_id is not None:
                conditions.append(BacktestSummary.skill_id == skill_id)
            else:
                conditions.append(BacktestSummary.skill_id.is_(None))
