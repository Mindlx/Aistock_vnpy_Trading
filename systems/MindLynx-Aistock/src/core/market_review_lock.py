...
def _is_lock_file_expired(lock_path: Path) -> bool:
    ...
    return datetime.now() - modified_at > timedelta(seconds=_MARKET_REVIEW_LOCK_STALE_TTL_SECONDS)
...
def _is_stale_lock(lock_path: Path) -> bool:
    ...
    return datetime.now() - started_at > timedelta(seconds=_MARKET_REVIEW_LOCK_STALE_TTL_SECONDS)
