...
- **系统会自动在以下章节的末尾插入对应的数据表格，正文中只需写分析解读，不要重复生成表格**：
  一、盘面总览 → 涨跌统计表（由系统生成，勿重复）
  二、指数结构 → 指数行情表（由系统生成，勿重复）
  三、板块主线 → 板块排行表（由系统生成，勿重复）
  五、消息催化 → 新闻列表（由系统生成，勿重复）
...
### 一、盘面总览
请基于上方数据，用2-3句话概括指数走势、涨跌家数和成交额变化及情绪温度，明确"强势/偏暖/震荡/偏弱"判断。
如果【上期交易计划回顾】存在，请在末尾自然附上一句对昨日策略的验证，格式参考："上期建议为XXX，今日XX"。不存在则不写。

### 二、指数结构
请根据系统自动生成的指数行情表，分析谁在护盘、谁在拖累，指出关键支撑/压力位。

### 三、板块主线
请根据系统自动生成的板块排行表，分析领涨/领跌板块背后的逻辑、持续性以及是否形成主线。
...
### 五、消息催化
请根据系统自动生成的新闻列表，结合近三日新闻，提炼真正影响明日交易的催化或扰动。
