...
class AgentOrchestrator:
    ...
    def _run_stage_agent(
        ...
        if timeout_seconds is not None and timeout_seconds > 0 and self._agent_run_accepts_timeout(agent.run):
    ...
    def _execute_pipeline(
        ...
        while index < len(agents):
            ...
            timeout_exhausted = timeout_s and remaining_budget is not None and remaining_budget <= 0
            budget_guard_triggered = (
                timeout_s and remaining_budget is not None and index > 0 and remaining_budget < stage_min_budget_s
            )
            ...
            if result.status == StageStatus.FAILED:
                non_critical = agent.agent_name in ("intel", "risk") or agent.agent_name in getattr(
                ...
                if not non_critical:
                    ...
                    logger.warning(
                        "[Orchestrator] stage '%s' failed (non-critical, degrading): %s", agent.agent_name, result.error
                    )
    ...
    def _build_specialist_agents(self, ctx: AgentContext) -> list:
        ...
        try:
            ...
            agents = []
            max_concurrent = max(1, int(getattr(self.config, 'agent_skill_max_concurrent', 5) or 5))
            for skill_id in selected[:max_concurrent]:
    ...
    def _normalize_dashboard_payload(
        ...
        dashboard_block = {} if not isinstance(dashboard_block, dict) else dict(dashboard_block)

        core = dashboard_block.get("core_conclusion")
        core = {} if not isinstance(core, dict) else dict(core)

        intelligence = dashboard_block.get("intelligence")
        intelligence = {} if not isinstance(intelligence, dict) else dict(intelligence)

        battle = dashboard_block.get("battle_plan")
        battle = {} if not isinstance(battle, dict) else dict(battle)
        ...
        trend_prediction = _first_non_empty_text(
            ...
            (getattr(base_opinion, "raw_data", {}) or {}).get("trend_summary") if base_opinion is not None else "",
        )
        if not trend_prediction:
            ...
            if ma_alignment or trend_score is not None:
                trend_prediction = (
                    f"技术面{ma_alignment or 'neutral'}，趋势评分 {trend_score if trend_score is not None else 'N/A'}"
                )
            else:
        ...
        sniper = {} if not isinstance(sniper, dict) else dict(sniper)
        ...
        sniper.setdefault(
            "stop_loss",
            key_levels.get("stop_loss") or key_levels.get("strong_support_stop_loss") or "待补充",
        )
        sniper.setdefault(
            "take_profit",
