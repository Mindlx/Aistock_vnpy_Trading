...
``sensitivity_score``, and the natural-language fields instead of inventing
new decision_type values.

## Confidence & Uncertainty

- ``confidence_level`` must be one of: high / medium-high / medium / medium-low / low
- Add ``confidence_rationale`` to the dashboard: one sentence explaining WHY this confidence level
  (e.g., "因子信号一致但新闻方向混乱" or "多时间框架信号共振支持当前判断")
- When signals conflict (LLM vs factor, short-term vs long-term), confidence MUST be downgraded
- If the current market regime is rare (<30% of recent 30 days), add ``regime_ood_warning``
  to the dashboard: "当前市场状态({regime})在过去30天内出现较少，历史规律参考价值有限"

## Risk Scenarios

In the ``dashboard``, include a ``scenarios`` object with 3 future scenarios:
1. **base** (most likely, ~60%): if current trend continues
2. **upside** (optimistic, ~20%): key catalyst fires
3. **downside** (pessimistic, ~15%): key risk materializes

Each scenario: ``{{"trigger": "...", "price_range": "...", "action": "..."}}``
Label all scenario probabilities as "主观估计，非统计概率"。

## Devil's Advocate Challenge

Before finalizing your decision, check yourself:
1. What is the strongest counter-argument to your conclusion?
2. What assumption are you least confident about?
3. What evidence would prove you wrong?

Add a ``devils_advocate`` field to the dashboard with these 3 answers (≤2 sentences each).
If the counter-argument is compelling, your confidence_level should reflect that.
"""
        if report_language == "en":
            return (
                prompt
