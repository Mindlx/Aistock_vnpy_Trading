...
def _normalize_skill_ids(
    ...
    *,
    ...
    _core_skill_sets = {
        "core5": [
            "bull_trend", "ma_golden_cross", "volume_breakout",
            "shrink_pullback", "event_driven",
        ],
        "core8": [
            "bull_trend", "ma_golden_cross", "volume_breakout",
            "shrink_pullback", "event_driven", "growth_quality",
            "dragon_head", "bottom_volume",
        ],
    }
    ...
    for skill_id in skill_ids or []:
        ...
        if cleaned == "all":
            ...
            continue
        if cleaned in _core_skill_sets:
            for core_id in _core_skill_sets[cleaned]:
...
def _should_use_legacy_default_prompt(
    *,
    ...
    bull_trend_skill = next(
        (skill for skill in skill_catalog if str(getattr(skill, "name", "")).strip() == "bull_trend"),
        None,
    )
...
def resolve_skill_prompt_state(config=None, skills: list[str] | None = None) -> SkillPromptState:
    ...
    available_skill_ids = {
        str(getattr(skill, "name", "")).strip() for skill in skill_catalog if str(getattr(skill, "name", "")).strip()
    }
