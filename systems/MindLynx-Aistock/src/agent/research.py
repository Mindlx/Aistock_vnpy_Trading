...
class ResearchAgent:
    ...
    def research(
        ...
        if progress_callback:
            progress_callback(
                {"type": "research_phase", "phase": "decompose", "message": "Decomposing research query..."}
            )
        ...
        for i, question in enumerate(questions):
            ...
            if tokens_used >= self.token_budget:
                logger.warning(
                    "[ResearchAgent] token budget exceeded (%d/%d), stopping", tokens_used, self.token_budget
                )
                break
        ...
        if progress_callback:
            progress_callback(
                {"type": "research_phase", "phase": "synthesize", "message": "Synthesising research report..."}
            )
...
"""
    ...
    def _synthesise_report(
        ...
        findings_text = "\n\n".join(
            f"### Sub-question: {f['question']}\n{f.get('content', 'No data')}" for f in findings if f.get("content")
        )
