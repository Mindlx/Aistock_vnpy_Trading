...
class LLMToolAdapter:
    ...
    def _init_litellm(self) -> None:
        ...
        if not keys:
            logger.info(f"Agent LLM: litellm initialized (model={litellm_model}, API key from environment)")
            return
        ...
        if len(keys) > 1:
            ...
            logger.info(f"Agent LLM: Legacy Router initialized with {len(keys)} keys for {litellm_model}")
        else:
        ...
        for idx, model in enumerate(models_to_try):
            ...
            try:
                ...
                if isinstance(e, _resolve_litellm_exception("RateLimitError")):
                    ...
                    should_backoff = idx + 1 < len(models_to_try) and providers[idx] == providers[idx + 1]
                    if should_backoff:
    ...
    def _call_litellm_model(
        ...
        *,
        ...
        uses_router = bool(use_channel_router and self._router and model in _router_model_names) or bool(
    ...
    def _parse_litellm_response(self, response: Any, model: str) -> LLMResponse:
            ...
            for tc in choice.message.tool_calls:
                ...
                if psf is not None:
                    sig = (
                        ...
                        else getattr(psf, "thought_signature", None)
                    )
                else:
                    func_psf = getattr(tc.function, "provider_specific_fields", None)
                    if func_psf is not None:
                        sig = (
                            func_psf.get("thought_signature")
                            if isinstance(func_psf, dict)
                            else getattr(func_psf, "thought_signature", None)
                        )
                    else:
        ...
        if response.usage:
            usage = {
                ...
                # DeepSeek prefix cache metrics (0 if provider doesn't support)
