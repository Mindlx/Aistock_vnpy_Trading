...
class SkillAggregator:
    ...
    def aggregate(
        ...
        weighted_score = (
            sum(_SIGNAL_SCORES.get(op.signal, 3.0) * weight for op, weight in zip(skill_opinions, weights))
            / total_weight
        )
        weighted_confidence = sum(op.confidence * weight for op, weight in zip(skill_opinions, weights)) / total_weight
        ...
        return AgentOpinion(
            ...
            raw_data={
                ...
                "individual_signals": {
                    op.agent_name: {"signal": op.signal, "confidence": op.confidence} for op in skill_opinions
                },
            },
        )
