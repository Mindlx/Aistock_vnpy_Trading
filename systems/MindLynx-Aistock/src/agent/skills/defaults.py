...
### 8. 技能与基线的关系（S4 修复）
- 以上基线为默认风险控制底线。**特定技能可根据其 instructions 对基线进行合理偏离**（如龙头股放宽乖离率至 7%、反转策略不要求多头排列等），偏离时必须在分析报告中注明依据。
"""
...
def _pick_primary_default_skill_id(candidates: list[object]) -> str:
    preferred = [
        str(getattr(skill, "name", "")).strip() for skill in candidates if bool(getattr(skill, "default_active", False))
    ]
    if preferred:
        return preferred[0]
...
def get_default_router_skill_ids(
    ...
    preferred = [
        str(getattr(skill, "name", "")).strip() for skill in candidates if bool(getattr(skill, "default_router", False))
    ]
    if preferred:
...
def get_regime_skill_ids(
    ...
    if regime_name:
        ...
        for skill in candidates:
            ...
            normalized_regimes = {str(item).strip().lower() for item in market_regimes if str(item).strip()}
