...
def load_skill_from_yaml(filepath: str | Path) -> Skill:
    ...
    if missing:
        raise ValueError(f"Skill file {filepath.name} missing required fields: {missing}")
    ...
    return Skill(
        ...
        market_regimes=(
            _coerce_string_list(data.get("market_regimes")) or _coerce_string_list(data.get("market-regimes"))
        ),
...
def load_skill_from_markdown(filepath: str | Path) -> Skill:
    ...
    display_name = str(metadata.get("display_name") or metadata.get("title") or skill_name).strip()
    description = str(metadata.get("description") or _infer_skill_description(instructions)).strip()
    ...
    return Skill(
        ...
        market_regimes=(
            _coerce_string_list(metadata.get("market-regimes")) or _coerce_string_list(metadata.get("market_regimes"))
        ),
...
class SkillManager:
    ...
    def get_skill_instructions(self, compact: bool = False) -> str:
        """Generate combined instruction text for all active skills.

        Args:
            compact: If True, render each skill as a one-liner (~10x smaller).
        ...
        if compact:
            return self._get_skill_instructions_compact(active)
        ...
        for cat_key in ordered_keys + [k for k in grouped if k not in ordered_keys]:
            ...
            for skill in skills_in_cat:
                ...
                if skill.bundle_dir and skill.entrypoint.endswith("SKILL.md"):
                    support_ref = "（bundle）"
                parts.append(
                    f"### 技能 {idx}: {skill.display_name} {rules_ref}{support_ref} (优先级: {skill.default_priority})\n\n"
    ...
    def _get_skill_instructions_compact(self, active: list[Skill]) -> str:
        """Render all active skills as compact one-liners (~85-95% smaller)."""
        lines = ["## 激活策略 (Compact)"]
        for skill in active:
            instr = skill.instructions or skill.description or ""
            # Skip title lines (bold markers, headings) to get actual content
            meaningful = ""
            for line in instr.split("\n"):
                ...
                break
            if not meaningful:
                meaningful = skill.description or skill.display_name
            if len(meaningful) > 100:
                meaningful = meaningful[:97] + "..."
            lines.append(f"- **{skill.display_name}**({skill.category}): {meaningful}")
        return "\n".join(lines)
