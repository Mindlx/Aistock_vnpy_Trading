...
def _is_non_retriable_tool_result(result: Any) -> bool:
    ...
    return isinstance(result, dict) and bool(result.get("error")) and result.get("retriable") is False
    ...
    if len(tool_calls) == 1:
        ...
        try:
            ...
            for future in as_completed(
                futures,
                timeout=(
                    tool_wait_timeout_seconds if tool_wait_timeout_seconds and tool_wait_timeout_seconds > 0 else None
                ),
            ):
                ...
                if progress_callback:
                    progress_callback(
                        {"type": "tool_done", "step": step, "tool": tc_item.name, "success": success, "duration": dur}
                    )
            ...
            timeout_triggered = True
            timeout_label = (
                f"{tool_wait_timeout_seconds:.2f}s" if tool_wait_timeout_seconds is not None else "the configured limit"
            )
