...
class AgentExecutor:
    ...
    def run(self, task: str, context: dict[str, Any] | None = None) -> AgentResult:
        ...
        prompt_template = LEGACY_DEFAULT_AGENT_SYSTEM_PROMPT if self.use_legacy_default_prompt else AGENT_SYSTEM_PROMPT
    ...
    def chat(
        ...
        market_guidelines = get_market_guidelines(stock_code, report_language)
        from src.core.prompt_shared import SCORING_CRITERIA, ACTION_GUARDRAILS
        prompt_template = LEGACY_DEFAULT_CHAT_SYSTEM_PROMPT if self.use_legacy_default_prompt else CHAT_SYSTEM_PROMPT
        system_prompt = prompt_template.format(
            market_role=market_role,
            market_guidelines=market_guidelines,
            default_skill_policy_section=default_skill_policy_section,
    ...
    def _build_user_message(self, task: str, context: dict[str, Any] | None = None) -> str:
        ...
        if context:
            ...
            if context.get("ly_signal"):  # @calibration LY量化信号注入
                parts.append(f"\n[系统已获取的量化预判信号（LY双模型）]\n{context['ly_signal']}")
