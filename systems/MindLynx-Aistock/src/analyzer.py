...
def _infer_trend_direction(trend: dict[str, Any]) -> str:
    ...
    combined = " ".join(
        str(trend.get(key, "")).strip() for key in ("trend_status", "ma_alignment") if str(trend.get(key, "")).strip()
    )
    if not combined:
        return "neutral"
    ...
    has_bullish = (
        ...
        or ("ma5>ma10" in normalized and any(pattern in normalized for pattern in ("ma10≤ma20", "ma10<=ma20")))
    )
    has_bearish = (
...
def stabilize_decision_with_structure(
    ...
    try:
        ...
        near_resistance = resistance is not None and not breakout and current_price >= resistance * 0.97
        mid_range = (
            support is not None and resistance is not None and support * 1.03 < current_price < resistance * 0.97
        )
...
def get_stock_name_multi_source(stock_code: str, context: dict | None = None, data_manager=None) -> str:
    """
    多来源获取股票中文名称
...
"""
    ...
    def _get_analysis_system_prompt(self, report_language: str, stock_code: str = "") -> str:
        ...
        base_prompt = (
                ...
                .replace("{scoring_criteria}", SCORING_CRITERIA)
                .replace("{action_guardrails}", ACTION_GUARDRAILS)
            )
        if lang == "en":
            return (
                base_prompt
...
"""
    ...
    def _init_litellm(self) -> None:
        ...
        if self._has_channel_config(config):
            ...
            try:
                ...
                unique_models = list(dict.fromkeys(e["litellm_params"]["model"] for e in model_list))
                logger.info(
        ...
        if len(legacy_model_list) > 1:
            ...
            try:
                ...
                logger.info(
                    f"Analyzer LLM: Legacy Router initialized with {len(legacy_model_list)} keys for {litellm_model}"
                )
                return
    ...
    def _normalize_usage(self, usage_obj: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "prompt_cache_hit_tokens": _get_value("prompt_cache_hit_tokens"),
            "prompt_cache_miss_tokens": _get_value("prompt_cache_miss_tokens"),
        }
        ...
        *,
        ...
        for model in models_to_try:
            ...
            try:
                ...
                uses_router = (use_channel_router and self._router and model in router_model_names) or (
                    self._router and model == config.litellm_model and not use_channel_router
                )
                if not uses_router:
                    try:
                        keys = get_api_keys_for_model(model, config)
    ...
    def analyze(
        ...
        if not self.is_available():
            return AnalysisResult(
                ...
                risk_warning=(
                    "Configure an LLM API key (GEMINI_API_KEY/ANTHROPIC_API_KEY/OPENAI_API_KEY) and retry."
                    if report_language == "en"
        ...
        try:
            ...
            # 设置生成配置（使用分析专用温度，研究推荐0.2-0.3）
            analysis_temp = getattr(config, "agent_analysis_temperature", None)
            if analysis_temp is None:
                analysis_temp = config.llm_temperature
            generation_config = {
...
| 市盈率(动态) | {rt.get("pe_ratio") if rt.get("pe_ratio") is not None else "N/A"} | |
| 市净率 | {rt.get("pb_ratio") if rt.get("pb_ratio") is not None else "N/A"} | |
| 总市值 | {self._format_amount(rt.get("total_mv")) if rt.get("total_mv") is not None else "N/A"} | |
| 流通市值 | {self._format_amount(rt.get("circ_mv"))} | |
| 60日涨跌幅 | {rt.get("change_60d", "N/A")}% | 中期表现 |
|"""

        # 添加所属概念/板块
        concept_ctx = context.get("concept_context", "")
        if concept_ctx:
            prompt += f"""### 📂 所属概念/板块
{concept_ctx}
...
"""
                ...
                bias_warning = "🚨 偏离较大，需谨慎评估追高风险" if trend.get("bias_ma5", 0) > 5 else "✅ 位置相对可控"
                prompt += f"""
### 技术与结构分析（供激活技能判断参考）
| 指标 | 数值 | 说明 |
|------|------|------|
...
"""
        ...
        # 添加支撑/阻力位（基于数学模型，替代 LLM 硬猜）
        sr_text = context.get("sr_levels", "")
        if sr_text:
            prompt += f"""### 📊 关键支撑/阻力位
{sr_text}
...
"""
        ...
        # O6: inject daily intelligence (all stocks, not per-stock)
        daily_intel = context.get("daily_intel_context", "")
        if daily_intel:
            prompt += "\n---\n" + daily_intel + "\n"

        # inject market background from latest market review
        market_bg = context.get("market_background", "")
        if market_bg:
            prompt += "\n---\n" + market_bg + "\n"
...
- **执行方案（必须完整填写，禁止留空）**：
  ├─ 建仓条件：乖离率是否满足 <5%、趋势是否多头排列等（写明符合/不符合的具体数值）
  ├─ 建仓价位：精确到分
  ├─ 止损价位：精确到分，并写明基于什么（MA20/结构低点/ATR倍数）
  ├─ 目标价位：精确到分
  ├─ 建议仓位：0 成 / 0.5 成 / 1 成 / 1.5 成 / 2 成 / 2.5 成
  └─ 风险回报比：止损空间 vs 目标空间的比例
- **实战原则验证（每项必须明确回答 ✅/⚠️/❌）**：
  ├─ 不追高：当前乖离率是否 <5%？
  ├─ 严进策略：买入条件满足几项？（bull_trend 定义的 6 项中）
  └─ 严格止损：是否已给出明确的止损价位？
- **检查清单**：每项用 ✅/⚠️/❌ 标记
...
- **信号相关性认知（重要）**：以下技术指标、定量因子 z-scores、体制分类均源自相同的基础行情数据（OHLCV，尤其是收盘价和成交量）。它们并非独立的验证来源，而是同一数据的不同统计变换。高相关信号组包括：**趋势组**（均线排列、momentum_reversal/spread、regime 趋势分类、consecutive_direction）共享趋势特征；**波动/风险组**（low_volatility、volatility_ratio、regime 波动分类、max_effect）共享波动率特征；**位置组**（price_position、乖离率 bias_ma5/ma10）共享相对位置特征。当同组内多个信号方向一致时，这是**额外的确认（约 1.2-1.5 倍的证据强度）而非独立的多次验证**——请勿将其视为“多个独立信号同时看多/看空”。跨组之间（趋势组 vs 波动组 vs 位置组）的信号具有更高的独立性，可给予相对更高的权重。
- **成交量信号的多机制区分（重要）**：成交量衍生的多个信号并非相互矛盾——它们测量的是**不同的市场机制**。**逆向/情绪类**（turnover_sentiment 换手率情绪因子、emotion_cycle 情绪周期策略）将高成交量解读为"散户追涨/情绪过热"→ 偏空/谨慎；**趋势跟随/量价确认类**（volume_breakout 放量突破策略、volume_status 放量上涨、bottom_volume 底部放量策略、放量拉升监控）将高成交量解读为"突破确认/主力介入/恐慌出清"→ 偏多。这两种解读并不互斥——高成交量同时意味着"散户参与度高"（逆向偏空）和"趋势获得确认"（趋势偏多）。请在分析中分别评估这两种机制，而非将其视为矛盾信号。
- **支撑/阻力位使用规则**：提示中已包含系统计算的关键支撑/阻力位（基于 OHLCV 聚类）。请直接使用这些数值填充 `support_level` 和 `resistance_level` 字段，而非自行重新推导。若系统未能提供，再自行计算。
- **策略规则与原始数据的关系**：提示中已包含原始行情数据（OHLCV、均线值、乖离率、量比、换手率等）。策略 YAML 中定义的触发条件（如"MA5 > MA10 > MA20"、"量比 > 2.0"）可以直接对照原始数据进行判断，无需重复检查。请勿将"对照数据检查策略条件"视为独立于原始数据的额外分析步骤——这是同一数据的直接应用。
- **因子评分与趋势分析的关系**：因子 z-scores（FactorEngine）与趋势评分（StockTrendAnalyzer）均源自相同的 OHLCV 数据，但代表了**两种不同的评分体系**——因子层是 IC/IR 经验校准的"定量笼子"（划定评分区间），趋势层是用户交易理念驱动的"规则信号"。请将它们作为两个独立信息源处理，而非两个相同的确认。当两者一致时，置信度适度提升；当两者分歧时，这正是需要综合判断的信号——而非系统错误。
- **因子评分与趋势评分的权重指引**：composite_score（因子笼）在**方向性判断**上具有更高的统计可靠性（IC/IR 经验校准），但在**具体入场时机**上应参考 signal_score（趋势规则包含乖离率、量能等时效信号）。两者冲突时：方向性以因子为主导，时机性以趋势为主导。
- **operation_advice 与因子方向一致**：你的 operation_advice 文本应与综合因子评分（composite_score）的方向基本一致。综合因子偏多（> +0.2）时用"买入/加仓"，偏空（< -0.2）时用"卖出/减仓"，中性时用"持有/观望"。如果你从新闻/消息面获得了因子无法捕获的关键信息（如突发利好/利空），可以偏离此指引，但必须在 analysis_summary 中解释原因。
...
"""
        ...
        # 当同组内多信号一致时，置信度应适度提升（~1.2-1.5×）而非翻倍
        signal_correlation_notice = """
---
## ⚠️ 信号相关性提示

以上定量因子、体制状态、仓位建议均源自相同的基础行情数据（OHLCV）。
信号分组与解读规则详见上方「决策仪表盘要求」中的「信号相关性认知」条目。
核心原则：同组内多信号一致 = 额外确认（~1.2-1.5×），跨组一致 = 更强证据（~1.5-2.0×）。
"""
        ...
        # 注入ATR仓位建议 (Phase 3)
        position_text = context.get("position_prompt", "")
        if position_text:
            prompt += "\n\n---\n\n" + str(position_text) + "\n"

        # 注入组合优化建议 (Phase 4)
        allocation_text = context.get("allocation_prompt", "")
        if allocation_text:
            prompt += "\n\n---\n\n" + str(allocation_text) + "\n"
        ...
        if knowledge_text:
            prompt += "\n\n---\n\n" + str(knowledge_text) + "\n"
            prompt += "\n> 📋 对比指令：请将上述历史背景与当前数据做比较分析。如历史结论与当前数据存在重大冲突（如上次看空、本次看多），请在 analysis_summary 中备注说明原因及判断依据。**重要**：上述历史分析分数是 AI 自身之前的判断——仅作为背景参考。应以当前 OHLCV 衍生数据（因子得分、趋势分析、体制分类）为主要判断依据。若历史判断与当前数据矛盾，优先跟随当前数据。"
