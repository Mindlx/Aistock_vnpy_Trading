    ...
    def _send_feishu_message(self, content: str, *, timeout_seconds: float | None = None) -> bool:
        ...
        def _post_payload(payload: dict[str, Any]) -> bool:
            ...
            response = requests.post(
                self._feishu_url, json=request_payload, timeout=timeout_seconds or 30, verify=self._webhook_verify_ssl
            )
        ...
        card_payload = {
            ...
            "card": {
                "config": {"wide_screen_mode": True},
                "header": {"title": {"tag": "plain_text", "content": "股票智能分析报告"}},
                "elements": [{"tag": "div", "text": {"tag": "lark_md", "content": prepared_content}}],
            },
        }
        ...
        # 2) 回退为普通文本消息
        text_payload = {"msg_type": "text", "content": {"text": prepared_content}}
