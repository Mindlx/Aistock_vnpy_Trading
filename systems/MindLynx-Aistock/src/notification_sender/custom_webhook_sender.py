...
class CustomWebhookSender:
    ...
    def _send_custom_webhook_image(self, image_bytes: bytes, fallback_content: str = "") -> bool:
            ...
            try:
                if self._is_discord_webhook(url):
                    ...
                    response = requests.post(
                        url, data=data, files=files, headers=headers, timeout=30, verify=self._webhook_verify_ssl
                    )
                    ...
                    if fallback_content:
                        ...
                        if self._post_custom_webhook(url, payload, timeout=30):
                            logger.info("自定义 Webhook %d（图片不支持，回退文本）推送成功", i + 1)
                            success_count += 1
                    else:
                        logger.warning("自定义 Webhook %d 不支持图片，且无回退内容，跳过", i + 1)
            except Exception as e:
    ...
    def _build_custom_webhook_template_payload(self, content: str) -> dict | None:
        ...
        if not isinstance(payload, dict):
            logger.error("CUSTOM_WEBHOOK_BODY_TEMPLATE 必须渲染为 JSON object，已回退为默认 Webhook payload")
            return None
        return payload
