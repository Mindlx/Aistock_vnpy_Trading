...
# Domain-split Config base classes (stubs for future field extraction)
from src.config_analysis import AnalysisConfig  # noqa: F401
from src.config_data import FUNDAMENTAL_STAGE_TIMEOUT_SECONDS_DEFAULT, DataConfig  # noqa: F401
from src.config_llm import AGENT_MAX_STEPS_DEFAULT, LLMConfig  # noqa: F401
from src.config_notification import NotificationConfig  # noqa: F401
...
class Config(LLMConfig, NotificationConfig, DataConfig, AnalysisConfig):
    ...
    # Analysis-specific temperature override (0=deterministic, 0.3=low-random, 0.7=creative)
    # Uses research-backed recommendation: lower temp for quantitative financial analysis
    ...
    def _load_from_env(cls) -> "Config":
        ...
        openai_base_url = os.getenv("OPENAI_BASE_URL") or ("https://aihubmix.com/v1" if _aihubmix else None)
        ...
        if not litellm_model:
            ...
            if gemini_api_keys:
                ...
                litellm_model = f"openai/{_openai_model_name}" if "/" not in _openai_model_name else _openai_model_name
        ...
        instance = cls(
            ...
            agent_skill_max_concurrent=parse_env_int(
                os.getenv("AGENT_SKILL_MAX_CONCURRENT"),
                5,
                field_name="AGENT_SKILL_MAX_CONCURRENT",
                minimum=1,
            ),
            agent_skill_compact=os.getenv("AGENT_SKILL_COMPACT", "true").lower() == "true",
            agent_analysis_temperature=float(os.getenv("AGENT_ANALYSIS_TEMPERATURE", "0.3")),
            ...
            event_monitor_check_interval=parse_env_int(
                os.getenv("EVENT_MONITOR_CHECK_INTERVAL"),
                300,
                field_name="EVENT_MONITOR_CHECK_INTERVAL",
                minimum=30,
            ),
            event_monitor_importance_threshold=parse_env_int(
                os.getenv("EVENT_MONITOR_IMPORTANCE_THRESHOLD"),
                7,
                field_name="EVENT_MONITOR_IMPORTANCE_THRESHOLD",
                minimum=1,
                maximum=10,
            ),
            event_monitor_stock_codes=[
                s.strip() for s in os.getenv("EVENT_MONITOR_STOCK_CODES", "").split(",") if s.strip()
            ],
            ...
            email_sender_name=os.getenv("EMAIL_SENDER_NAME", "MindLynx-Aistock分析助手"),
            ...
            notification_report_channels=parse_notification_route_channels(os.getenv("NOTIFICATION_REPORT_CHANNELS")),
            notification_alert_channels=parse_notification_route_channels(os.getenv("NOTIFICATION_ALERT_CHANNELS")),
            ...
            backtest_report_enabled=parse_env_bool(os.getenv("BACKTEST_REPORT_ENABLED"), default=True),
            backtest_report_output_dir=os.getenv("BACKTEST_REPORT_OUTPUT_DIR", "reports/backtest/"),
            ...
            # WebSocket 实时行情配置
            websocket_realtime_enabled=os.getenv("WEBSOCKET_REALTIME_ENABLED", "false").lower() == "true",
            websocket_reconnect_delay=parse_env_int(
                os.getenv("WEBSOCKET_RECONNECT_DELAY"), 5, field_name="WEBSOCKET_RECONNECT_DELAY", minimum=1
            ),
            # 盘中实时监控配置
            ...
            realtime_monitor_atr_multipliers=os.getenv("REALTIME_MONITOR_ATR_MULTIPLIERS", "2.0,2.5,3.0"),
            realtime_monitor_volume_ratio_threshold=parse_env_float(
                os.getenv("REALTIME_MONITOR_VOLUME_RATIO_THRESHOLD"),
                3.0,
                field_name="REALTIME_MONITOR_VOLUME_RATIO_THRESHOLD",
                minimum=0.1,
            ),
            realtime_monitor_price_change_threshold=parse_env_float(
                os.getenv("REALTIME_MONITOR_PRICE_CHANGE_THRESHOLD"),
                2.0,
                field_name="REALTIME_MONITOR_PRICE_CHANGE_THRESHOLD",
                minimum=0.1,
            ),
            ...
            fundamental_fetch_timeout_seconds=parse_env_float(
                ...
                10.0,  # 2026-07-06: Tushare+Tencent 补充链需 ~5s, 3s 不够
            ...
            factor_monitor_enabled=os.getenv("FACTOR_MONITOR_ENABLED", "true").lower() == "true",
            factor_monitor_output_dir=os.getenv("FACTOR_MONITOR_OUTPUT_DIR", "reports/factors/"),
        ...
        try:
            issues = instance.validate_structured()
            for issue in issues:
                log_func = logger.error if issue.severity == "error" else logger.warning
                log_func("Config validation [%s]: %s (field=%s)", issue.severity, issue.message, issue.field)
        except Exception:
            pass
        return instance
    ...
    def _resolve_realtime_source_priority(cls) -> str:
        ...
        default_priority = "efinance,tencent,akshare_sina,akshare_em"
    ...
    def validate_structured(self) -> list[ConfigIssue]:
        ...
        if not self.stock_list:
            ...
            configured_stock_set = {normalize_stock_code(code) for code in self.stock_list if (code or "").strip()}
        ...
        if available_router_model_set:
            ...
            issues.append(
                ConfigIssue(
                    severity="error",
                    message=("已配置 Agent 主模型，但未找到可用的运行时来源（启用渠道或匹配的 API Key）。"),
                    field="AGENT_LITELLM_MODEL",
                )
            )
        ...
        has_notification = bool(
            ...
            or (self.gotify_url and (self.gotify_token or "").strip() and _has_gotify_base_url(self.gotify_url))
        ...
        if self.gotify_url and _has_gotify_base_url(self.gotify_url) and not (self.gotify_token or "").strip():
            issues.append(
                ConfigIssue(
                    severity="warning",
        ...
        if self.notification_min_severity and not is_supported_notification_severity(self.notification_min_severity):
            issues.append(
                ConfigIssue(
                    severity="error",
                    message=(f"通知最低级别配置无效，允许值：{', '.join(NOTIFICATION_SEVERITIES)}"),
        ...
        if self.notification_daily_digest_enabled:
            issues.append(
                ConfigIssue(
                    severity="warning",
                    message=("NOTIFICATION_DAILY_DIGEST_ENABLED 当前为预留配置；P4 不会发送每日摘要或持久化摘要内容。"),
