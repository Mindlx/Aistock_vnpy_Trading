...
class AskCommand(BotCommand):
    ...
    def _validate_single_code(self, code: str) -> str | None:
    ...
    def _resolve_skill_name(self, skill_id: str | None) -> str:
    ...
    def execute(self, message: BotMessage, args: list[str]) -> BotResponse:
        ...
        codes = self._parse_stock_codes(raw_code_str)
        skill_id = (
            self._parse_skill(["placeholder"] + remaining_args) if remaining_args else self._get_default_skill_id()
        )
    ...
    def _analyze_multi(
        ...
        def _run_one(stock_code: str) -> tuple[str, dict[str, Any] | None, str | None]:
            try:
    ...
    @staticmethod
    def _extract_stock_name(stock_code: str, dashboard: dict[str, Any] | None) -> str:
    ...
    @staticmethod
    def _extract_signal(dashboard: dict[str, Any] | None) -> str:
        if isinstance(dashboard, dict):
    ...
    @staticmethod
    def _extract_confidence(dashboard: dict[str, Any] | None) -> float | None:
    ...
    @staticmethod
    def _extract_summary(stock_code: str, dashboard: dict[str, Any] | None, raw_content: str) -> str:
        if isinstance(dashboard, dict):
    ...
    @staticmethod
    def _extract_risk_flags(dashboard: dict[str, Any] | None) -> list[dict[str, str]]:
        if not isinstance(dashboard, dict):
            return []
    ...
    @staticmethod
    def _format_sniper_value(value: Any) -> str | None:
        if value is None:
            return None
    ...
    @staticmethod
    def _format_stock_result(stock_code: str, dashboard: dict[str, Any] | None, raw_content: str) -> str:
        if not isinstance(dashboard, dict):
            content = raw_content
            if len(content) > 800:
                content = content[:800] + "\n... (已截断，完整分析请单独查询)"
