...
class StatusCommand(BotCommand):
    ...
    def _collect_status(self, config) -> dict:
        ...
        status["ai_channels"] = [
            str(channel.get("name") or "").strip() for channel in llm_channels if str(channel.get("name") or "").strip()
        ]
        status["ai_yaml"] = getattr(config, "llm_models_source", "") == "litellm_config" and bool(llm_model_list)
    ...
    def _format_status(self, status: dict, platform: str) -> str:
        ...
        lines.extend(
            [
                ...
                "• Legacy Key: "
                + ", ".join(f"{name}{icon(enabled)}" for name, enabled in status["ai_legacy_keys"].items()),
                "",
                "**🔍 搜索服务**",
