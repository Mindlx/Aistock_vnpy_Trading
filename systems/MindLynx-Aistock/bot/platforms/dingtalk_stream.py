...
class DingtalkStreamHandler:
    ...
    def _parse_stream_message(self, incoming: Any, raw_data: dict) -> BotMessage | None:
        """
        解析 Stream 消息为统一格式
...
class DingtalkStreamClient:
    ...
    def __init__(self, client_id: str | None = None, client_secret: str | None = None):
        ...
        if not DINGTALK_STREAM_AVAILABLE:
            raise ImportError("dingtalk-stream SDK 未安装。\n请运行: pip install dingtalk-stream")
    ...
    def start(self) -> None:
        ...
        self._client.register_callback_handler(dingtalk_stream.chatbot.ChatbotMessage.TOPIC, handler.create_handler())
    ...
    def start_background(self) -> None:
        ...
        self._background_thread = threading.Thread(
            target=self._run_in_background, daemon=True, name="DingtalkStreamClient"
        )
...
# 全局客户端实例
_stream_client: DingtalkStreamClient | None = None


def get_dingtalk_stream_client() -> DingtalkStreamClient | None:
    """获取全局 Stream 客户端实例"""
