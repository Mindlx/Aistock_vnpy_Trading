...
from bot.models import BotMessage, ChatType, WebhookResponse
...
class DiscordPlatform(BotPlatform):
    ...
    def __init__(self):
        ...
        config = get_config()
        self._interactions_public_key = (getattr(config, "discord_interactions_public_key", None) or "").strip()
    ...
    def handle_webhook(
        ...
        body: bytes,
        data: dict[str, Any],
    ) -> tuple[BotMessage | None, WebhookResponse | None]:
    ...
    def parse_message(self, data: dict[str, Any]) -> BotMessage | None:
        ...
        author = data.get("user") or (data.get("member") or {}).get("user") or data.get("author", {})
    ...
    def send_followup(self, response: Any, message: BotMessage) -> bool:
        ...
        if not application_id or not interaction_token:
            logger.warning("[Discord] 缺少 application_id 或 interaction token，无法发送 follow-up")
            return False
        ...
        try:
            chunks = chunk_content_by_max_words(content, self.DISCORD_MAX_CONTENT_LENGTH)
        except (ValueError, Exception) as exc:
            logger.warning("[Discord] 消息分块失败: %s，尝试整段发送", exc)
            chunks = [content]

        base_url = f"https://discord.com/api/v10/webhooks/{application_id}/{interaction_token}"
