...
class FeishuReplyClient:
        ...
        try:
            ...
            if message_id:
                ...
                request = (
                    ...
                    .request_body(
                        ReplyMessageRequestBody.builder().content(content_json).msg_type("interactive").build()
                    )
                    .build()
                )
    ...
    def reply_text(self, message_id: str, text: str, at_user: bool = False, user_id: str | None = None) -> bool:
        """
        回复文本消息（支持交互卡片和分段发送）
        ...
        if content_bytes > self._max_bytes:
            logger.info(f"[Feishu Stream] 回复消息内容超长({content_bytes}字节)，将分批发送")
        ...
        # 单条消息，使用交互卡片
        return self._send_interactive_card(formatted_text, message_id=message_id, at_user=at_user, user_id=user_id)

    def send_to_chat(self, chat_id: str, text: str, receive_id_type: str = "chat_id") -> bool:
        """
        ...
        content_bytes = len(formatted_text.encode("utf-8"))
        if content_bytes > self._max_bytes:
            logger.info(f"[Feishu Stream] 发送消息内容超长({content_bytes}字节)，将分批发送")
            return self._send_to_chat_chunked(
                formatted_text,
                lambda chunk: self._send_interactive_card(
                    chunk,
...
class FeishuStreamHandler:
    ...
    def __init__(self, on_message: Callable[[BotMessage], BotResponse], reply_client: FeishuReplyClient):
        """
        Args:
    ...
    def _log_incoming_message(self, message: BotMessage) -> None:
        ...
        self._logger.info(
            "[Feishu Stream] Incoming message: msg_id=%s user_id=%s chat_id=%s chat_type=%s content=%s",
            message.message_id,
            message.user_id,
            message.chat_id,
    ...
    def _parse_event_message(self, event: "P2ImMessageReceiveV1") -> BotMessage | None:
        """
        解析飞书事件消息为统一格式
...
class FeishuStreamClient:
    ...
    def __init__(self, app_id: str | None = None, app_secret: str | None = None):
        ...
        self._ws_client: ws.Client | None = None
        self._reply_client: FeishuReplyClient | None = None
        self._message_handler: FeishuStreamHandler | None = None
    ...
    def _create_event_handler(self) -> "lark.EventDispatcherHandler":
        ...
        # 创建消息处理器
        handler = FeishuStreamHandler(self._create_message_handler(), self._reply_client)
        ...
        event_handler = (
            lark.EventDispatcherHandler.builder(
                encrypt_key=encrypt_key, verification_token=verification_token, level=lark.LogLevel.WARNING
            )
...
# 全局客户端实例
_stream_client: FeishuStreamClient | None = None


def get_feishu_stream_client() -> FeishuStreamClient | None:
    """获取全局 Stream 客户端实例"""
