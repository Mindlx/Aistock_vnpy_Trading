...
class CommandDispatcher:
    ...
    def _prepare_dispatch(
    ...
    ) -> tuple[str | None, list[str], BotCommand | None, BotResponse | None]:
        ...
        if not self._rate_limiter.is_allowed(message.user_id):
            ...
            return None, [], None, BotResponse.error_response(f"请求过于频繁，请 {remaining_time} 秒后再试")
        ...
        if command is None:
            return (
                ...
                args,
                None,
                BotResponse.error_response(f"未知命令: {cmd_name}\n发送 `{self.command_prefix}help` 查看可用命令。"),
            )
        ...
        if error_msg:
            return cmd_name, args, None, BotResponse.error_response(f"{error_msg}\n用法: `{command.usage}`")
...
"""
    ...
    async def _try_nl_routing(self, message: BotMessage) -> BotResponse | None:
    ...
    def _try_nl_routing_sync(self, message: BotMessage) -> BotResponse | None:
    ...
    @staticmethod
    async def _parse_intent_via_llm(text: str, config) -> dict | None:
    ...
    @staticmethod
    def _parse_intent_via_llm_sync(text: str, config) -> dict | None:
    ...
    @staticmethod
    def _parse_intent_payload(raw: str) -> dict | None:
    ...
    @classmethod
    def _resolve_stock_code_from_text(cls, text: str) -> str | None:
        ...
        def _unique_partial_match(candidate: str) -> str | None:
            if not re.search(r"[\u4e00-\u9fff]", candidate):
                return None
            matches = [code for code, stock_name in STOCK_NAME_MAP.items() if candidate and candidate in stock_name]
