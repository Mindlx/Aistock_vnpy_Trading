...
class AkshareFetcher(BaseFetcher):
    ...
    def get_realtime_quote(self, stock_code: str, source: str = "em") -> UnifiedRealtimeQuote | None:
        """
        获取实时行情数据（支持多数据源）
    ...
    def get_sector_rankings(self, n: int = 5) -> tuple[list[dict], list[dict]] | None:
        ...
        def _get_rank_top_n(df: pd.DataFrame, change_col: str, industry_name: str, n: int,
                          amount_col: str | None = None) -> tuple[list, list]:
            ...
            for _, row in top.iterrows():
                d = {"name": row[industry_name], "change_pct": row[change_col]}
                if amount_col and amount_col in df.columns:
                    try:
        ...
        try:
            ...
            if df is not None and not df.empty:
                change_col = "涨跌幅"
                name = "板块名称"
                return _get_rank_top_n(df, change_col, name, n, amount_col="成交额")
        ...
        try:
            ...
            change_col = "涨跌幅"
            name = "板块"
            return _get_rank_top_n(df, change_col, name, n, amount_col="总成交额")
    ...
    def _get_eastmoney_hot_stocks(self, ak: Any, n: int = 10) -> list[dict[str, Any]] | None:
        """获取东方财富人气股榜。"""
        self._set_random_user_agent()
        self._enforce_rate_limit()
    ...
    def _get_eastmoney_hot_up_stocks(self, ak: Any, n: int = 10) -> list[dict[str, Any]] | None:
        """获取东方财富飙升榜。"""
        self._set_random_user_agent()
        self._enforce_rate_limit()
    ...
    def _get_xueqiu_hot_stocks(self, ak: Any, n: int = 10) -> list[dict[str, Any]] | None:
    ...
    @staticmethod
    def _find_column_containing(df: pd.DataFrame, keywords: tuple[str, ...]) -> str | None:
