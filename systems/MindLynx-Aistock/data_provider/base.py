...
import logging
import os
import random  # ⚠️ 被误删过导致 random_sleep NameError → daemon 崩溃 → FD 耗尽。删除前确认 efinance/akshare 不依赖此导入
import time
...
class BaseFetcher(ABC):
    ...
    def get_concept_rankings(self, n: int = 5) -> tuple[list[dict], list[dict]] | None:
        """
        获取概念/题材涨跌榜。
    ...
    def get_hot_stocks(self, n: int = 10) -> list[dict[str, Any]] | None:
        """
        获取市场人气股榜。
    ...
    def get_daily_data(
        self, stock_code: str, start_date: str | None = None, end_date: str | None = None, days: int = 30
    ) -> pd.DataFrame:
        """
        获取日线数据（统一入口）
        ...
        try:
            ...
            # Step 3.5: 质量验证（拒绝脏数据）
            if not self._validate_quality(df):
                raise DataFetchError(
                    f"[{self.name}] {stock_code} 数据质量验证失败: "
                    f"rows={len(df)}, "
                    f"close_min={df['close'].min() if 'close' in df.columns else 'N/A'}, "
                    f"volume_nunique={df['volume'].nunique() if 'volume' in df.columns else 'N/A'}"
                )
            ...
            from src.core.data_health import record_data_fetch

            record_data_fetch(self.name, True, elapsed * 1000)
            return df
            ...
            record_data_fetch(self.name, False)
    ...
    @staticmethod
    def _validate_quality(df: pd.DataFrame) -> bool:
        """快速质量验证，在 get_daily_data 中调用。
        ...
        if "volume" in df.columns and (df["volume"] <= 0).all():
            return False
        # 平线检测：检查闭盘价是否全相同（数据异常）
        if "close" in df.columns and df["close"].nunique() <= 2:
            return False
        return True
...
class DataFetcherManager:
    ...
    def __init__(self, fetchers: list[BaseFetcher] | None = None):
        ...
        self._consecutive_failures: dict[str, int] = {}  # adaptive priority tracking
    ...
    def _get_fetcher_by_name(self, fetcher_name: str, capability: str = "") -> BaseFetcher | None:
        self._ensure_concurrency_guards()
        with self._fetchers_lock:
    ...
    @staticmethod
    def _call_availability_probe(fetcher: BaseFetcher, probe_name: str, capability: str) -> bool | None:
    ...
    def _get_fundamental_cache_key(self, stock_code: str, budget_seconds: float | None = None) -> str:
    ...
    @staticmethod
    def _try_scalar_isna(value: Any, context: str) -> bool | None:
    ...
    def _try_parallel_fetch(
        ...
        """并行竞速：同时尝试多个数据源，取首个通过质量验证的结果。"""
        if not fetchers:
            return None
        import concurrent.futures

        with concurrent.futures.ThreadPoolExecutor(max_workers=min(3, len(fetchers))) as executor:
            future_map = {
                executor.submit(
            ...
            for future in concurrent.futures.as_completed(future_map, timeout=timeout):
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        if BaseFetcher._validate_quality(df):
                            for f in future_map:
                                if not f.done():
                                    f.cancel()
                            return df
    ...
    def _daily_cache_path(self, stock_code: str, days: int) -> str:
        """缓存文件路径。"""
    ...
    def _read_daily_cache(self, stock_code: str, days: int) -> pd.DataFrame | None:
        ...
        try:
            ...
            if time.time() - mtime > 7200:  # 2 小时过期
                return None
    ...
    def _write_daily_cache(self, stock_code: str, days: int, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            logger.debug("[缓存] 写入日线缓存失败 %s: %s", stock_code, exc)

    # ── 实时行情缓存兜底 ──────────────────────────────────────
    _REALTIME_CACHE_TTL = 180  # 3 分钟

    def _realtime_quote_cache_path(self, stock_code: str) -> str:
        cache_dir = os.path.join(os.path.expanduser("~/.hermes/data"), "realtime_cache")
        return os.path.join(cache_dir, f"{stock_code}.json")

    def _read_realtime_quote_cache(self, stock_code: str) -> dict | None:
        """读取实时行情缓存。TTL 内有效，超时返回 None。"""
        path = self._realtime_quote_cache_path(stock_code)
        try:
            if not os.path.exists(path):
                return None
            mtime = os.path.getmtime(path)
            if time.time() - mtime > self._REALTIME_CACHE_TTL:
                return None
    ...
    def _write_realtime_quote_cache(self, stock_code: str, quote: Any) -> None:
        """将实时行情写入文件缓存。"""
        try:
            path = self._realtime_quote_cache_path(stock_code)
            os.makedirs(os.path.dirname(path), exist_ok=True)
            ...
            if hasattr(q, "__dataclass_fields__"):
                q = {f.name: getattr(quote, f.name) for f in dataclasses.fields(quote)}
            elif hasattr(q, "_asdict"):
                q = q._asdict()
            elif not isinstance(q, dict):
                q = {"price": getattr(q, "price", None)}
            data = {"code": stock_code, "timestamp": time.time(), "quote": q}
            with open(path, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, default=str)
        except Exception as exc:
            logger.debug("[缓存] 写入实时行情缓存失败 %s: %s", stock_code, exc)
    ...
    def get_daily_data(
        ...
        # 缓存兜底：先检查最近缓存（2 小时内有效），全源失败时作为回退
        ...
        # 通用路径：优先并行竞速前 3 个数据源，加速获取
        try:
            parallel_result = self._try_parallel_fetch(
                fetchers[:3], stock_code,
                start_date=start_date, end_date=end_date, days=days,
            )
            if parallel_result is not None:
                elapsed = time.time() - request_start
                logger.info(
                    f"[数据源完成] {stock_code} 使用 [并行竞速] 获取成功: "
                    f"rows={len(parallel_result)}, elapsed={elapsed:.2f}s"
                )
                # 跨源对账: 并行竞速成功后，尝试第三取数器做价格交叉验证
                if len(fetchers) >= 2:
                    try:
                        df2 = self._call_fetcher_method(
                            fetchers[1], "get_daily_data",
                        ...
                        if df2 is not None and not df2.empty and 'close' in df2.columns:
                            last_close_1 = parallel_result['close'].iloc[-1]
                            last_close_2 = df2['close'].iloc[-1]
                            if last_close_1 > 0 and last_close_2 > 0:
                                pct_diff = abs(last_close_1 - last_close_2) / last_close_1 * 100
                                if pct_diff > 1.0:
                                    logger.warning(
                                        "[跨源对账] %s 收盘价差异: %s=%.2f vs %s=%.2f (差异%.1f%%)",
                                        stock_code, fetchers[0].name, last_close_1,
                                        fetchers[1].name, last_close_2, pct_diff,
                                    )
                    except Exception:
                        pass
                return parallel_result, fetchers[0].name
        except Exception:
            pass

        FETCHER_TIMEOUT = 5.0  # 每个数据源最多等5秒，超时跳过（防pytdx等慢源卡死）
        ...
        for attempt, fetcher in enumerate(fetchers, start=1):
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as pool:
                    ...
                    df = future.result(timeout=FETCHER_TIMEOUT)
                ...
                if df is not None and not df.empty:
                    ...
                    self._write_daily_cache(stock_code, days, df)
                    return df, fetcher.name

            except FuturesTimeoutError:
                error_msg = f"[{fetcher.name}] (TimeoutError) 超时 {FETCHER_TIMEOUT}s，跳过"
                logger.warning(
                    f"[数据源超时 {attempt}/{total_fetchers}] [{fetcher.name}] {stock_code}: "
                    f"超过 {FETCHER_TIMEOUT}s，跳过"
                )
                errors.append(error_msg)
        ...
        if cached is not None:
            logger.warning(
                f"[数据源兜底] {stock_code} 所有数据源失败，返回缓存数据 (标记延迟, "
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        if is_us or is_hk:
            ...
            if primary_quote is not None:
                # 实时行情新鲜度验证：价格 ≤ 0 或缺失时，标记为不可靠
                if primary_quote.price is None or primary_quote.price <= 0:
                    logger.warning("[实时行情] %s price=%s 无效，降级为历史收盘价参考", stock_code, primary_quote.price)
            ...
            try:
                ...
                if quote is not None and quote.has_basic_data():
                    if primary_quote is None:
                        ...
                        # Supplement missing fields from this source (先2个源补充，仍缺失则继续)
                        supplement_attempts += 1
                        if supplement_attempts > 2:
        ...
        if primary_quote is not None:
            self._write_realtime_quote_cache(stock_code, primary_quote)
            return primary_quote

        # 所有数据源都失败 → 尝试缓存兜底
        cached = self._read_realtime_quote_cache(stock_code)
        if cached:
            q = cached.get("quote", {})
            logger.info(f"[实时行情] {stock_code} 使用缓存兜底（{time.time() - cached.get('timestamp', 0):.0f}s 前）")
            # 重建 UnifiedRealtimeQuote，标记为缓存
            from .realtime_types import UnifiedRealtimeQuote, RealtimeSource
            try:
                return UnifiedRealtimeQuote(
                    code=stock_code, name=q.get("name", ""),
                    source=RealtimeSource.CACHE,
                    price=q.get("price"),
    ...
    def get_stock_name(self, stock_code: str, allow_realtime: bool = True) -> str | None:
        """
        获取股票中文名称（自动切换数据源）
    ...
    def _has_meaningful_payload(payload: Any) -> bool:
        ...
        if isinstance(payload, pd.DataFrame):
            if payload.empty:
                return False
            return any(DataFetcherManager._has_meaningful_payload(v) for v in payload.to_numpy().flat)
        ...
        if isinstance(payload, np.ndarray):
            if payload.ndim == 0:
                ...
                return any(DataFetcherManager._has_meaningful_payload(v) for v in payload.flat)
    ...
    def get_fundamental_context(self, stock_code: str, budget_seconds: float | None = None) -> dict[str, Any]:
        """
    ...
    def get_capital_flow_context(self, stock_code: str, budget_seconds: float | None = None) -> dict[str, Any]:
        """资金流向块（fail-open）。"""
        from src.config import get_config
    ...
    def get_dragon_tiger_context(self, stock_code: str, budget_seconds: float | None = None) -> dict[str, Any]:
        """龙虎榜块（fail-open）。"""
        from src.config import get_config
    ...
    def get_board_context(self, stock_code: str, budget_seconds: float | None = None) -> dict[str, Any]:
        """板块榜单块（fail-open）。"""
        from src.config import get_config
