...
class CircuitBreaker:
    ...
    def __init__(
        ...
        on_open_callback: Callable[[str, int, str | None], None] | None = None,
    ):
        ...
        try:
            ...
            for source, info in loaded.items():
                if isinstance(info, dict):
                    lft = info.get("last_failure_time", 0.0)
                    state = info.get("state", self.CLOSED)
                    # 跨交易日重置：如果熔断/半开状态的失败时间远大于冷却期（>10×），
                    # 说明是上个交易日的残留，重置为 CLOSED 避免开盘后被旧状态卡住
                    if state in (self.OPEN, self.HALF_OPEN) and lft > 0 and (now - lft) > self.cooldown_seconds * 10:
                        state = self.CLOSED
                    self._states[source] = {
                        "state": state,
                        "failures": info.get("failures", 0),
                        "last_failure_time": lft,
                        "half_open_calls": info.get("half_open_calls", 0),
                    }
        ...
        with self._lock:
            ...
            if state["state"] == self.HALF_OPEN:
                ...
                if self._on_open_callback:
                    self._on_open_callback(source, state["failures"], error)
...
# 全局熔断器实例（实时行情专用） — 带熔断告警回调
# cooldown 从 CIRCUIT_BREAKER_COOLDOWN 读取，默认 300s（5分钟）
_CB_COOLDOWN = float(os.environ.get("CIRCUIT_BREAKER_COOLDOWN", "300"))
...
def _write_circuit_breaker_alert(source: str, failures: int, error: str | None = None) -> None:
    """熔断器进入OPEN状态时写 alert_triggers 表（轻量sqlite3直写，无SQLAlchemy依赖）。"""
    try:
        db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "stock_analysis.db")
        if not os.path.exists(db_path):
            logger.debug("[熔断告警] DB不存在: %s", db_path)
            return
        conn = sqlite3.connect(db_path)
        now_str = datetime.now().isoformat(timespec="seconds")
        conn.execute(
            """INSERT INTO alert_triggers
               (rule_id, target, observed_value, threshold, reason, data_source, data_timestamp, triggered_at, status)
               VALUES (0, ?, ?, ?, ?, ?, ?, ?, 'open')""",
            (source, float(failures), 0.0, error or "", "circuit_breaker", now_str, now_str),
        )
        conn.commit()
        conn.close()
        logger.info("[熔断告警] alert_triggers写入: %s failures=%d %s", source, failures, error or "")
    except Exception as exc:
        logger.debug("[熔断告警] 写入失败: %s", exc)


# 注册熔断告警回调
_realtime_circuit_breaker._on_open_callback = _write_circuit_breaker_alert
_chip_circuit_breaker._on_open_callback = _write_circuit_breaker_alert
