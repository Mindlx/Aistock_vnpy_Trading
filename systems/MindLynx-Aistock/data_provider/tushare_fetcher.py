...
class TushareFetcher(BaseFetcher):
    ...
    def __init__(self, rate_limit_per_minute: int = 80):
        ...
        self.date_list: list[str] | None = None  # 交易日列表缓存（倒序，最新日期在前）
        self._date_list_end: str | None = None  # 缓存对应的截止日期，用于跨日刷新
    ...
    def _get_trade_dates(self, end_date: str | None = None) -> list[str]:
        ...
        try:
            ...
            logger.warning(f"[Tushare] trade_cal 调用失败 ({e})，跳过交易日历")
            self.date_list = []
    ...
    @staticmethod
    def _pick_trade_date(trade_dates: list[str], use_today: bool) -> str | None:
        """根据可用交易日列表选择当天或前一交易日。"""
        if not trade_dates:
            return None
    ...
    @staticmethod
    def _detect_exchange_hint(stock_code: str) -> str | None:
    ...
    def get_main_indices(self, region: str = "cn") -> list[dict] | None:
        """
    ...
    def get_trade_time(self, early_time="09:30", late_time="16:30") -> str | None:
        """
        获取当前时间可以获得数据的开始时间日期
    ...
    def get_chip_distribution(self, stock_code: str) -> ChipDistribution | None:
        ...
        try:
            ...
            if not start_date:
                # trade_cal 可能无权限，fallback 到最近交易日（cyq_chips免费版限5次/分钟）

                china_now = self._get_china_now()
                # 直接试最近1-2个自然日（周末跳过），minimize API调用次数
                start_date = (china_now - timedelta(days=1)).strftime("%Y%m%d")
                test_df = self._call_api_with_rate_limit(
                    "cyq_chips",
                    ts_code=self._convert_stock_code(stock_code),
                    start_date=start_date,
                    end_date=start_date,
                ...
                if test_df is None or test_df.empty:
                    start_date = (china_now - timedelta(days=2)).strftime("%Y%m%d")
                    test_df = self._call_api_with_rate_limit(
                        "cyq_chips",
                        ts_code=self._convert_stock_code(stock_code),
                        start_date=start_date,
                ...
                if test_df is not None and not test_df.empty:
                    logger.info(f"[Tushare] trade_cal 不可用，使用试错法找到可行日期 {start_date}")
                else:
                    logger.warning(f"[Tushare] 无法获取 {stock_code} 的可行筹码日期")
                    return None
