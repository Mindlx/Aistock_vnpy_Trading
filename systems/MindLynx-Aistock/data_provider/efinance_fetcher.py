...
class EfinanceFetcher(BaseFetcher):
    ...
    priority = int(os.getenv("EFINANCE_PRIORITY", "1"))  # 东财HTTP，排在PytdxFetcher之后
    ...
    def get_realtime_quote(self, stock_code: str) -> UnifiedRealtimeQuote | None:
        ...
        try:
            ...
            circuit_breaker.record_failure(source_key, str(e)[:80])
            return None
    ...
    def get_main_indices(self, region: str = "cn") -> list[dict[str, Any]] | None:
        ...
        try:
            ...
            # 快速路径：直接调用新浪免费接口获取指数（稳定可靠，避免 efinance 的 push2 断连问题）
            try:
                import requests as _req

                sina_codes = "s_sh000001,s_sz399001,s_sz399006,s_sh000688,s_sh000016,s_sh000300"
                sina_resp = _req.get(
                ...
                if sina_resp.status_code == 200:
                    sina_results = []
                    for line in sina_resp.text.strip().split("\n"):
                        if not line.strip():
                            continue
                        # Parse: var hq_str_s_sh000001="上证指数,4112.90,4096.17,4120.09,4067.75,0.00,0.00,..."
                        eq_idx = line.find('"')
                        if eq_idx < 0 or line.rfind('"') <= eq_idx:
                            continue
                        fields = line[eq_idx + 1 : line.rfind('"')].split(",")
                        if len(fields) < 10:
                            continue
                        # Extract sina code from var name: s_sh000001 -> sh000001
                        var_part = line.split("=")[0].strip()
                        sina_sym = var_part.split("_")[-1] if "_" in var_part else ""
                        # Map to full_code
                        code_to_full = {
                        ...
                        match = code_to_full.get(sina_sym)
                        if not match:
                            continue
                        full_code, name = match
                        ...
                        sina_results.append({
                            ...
                            "volume": volume,
                            "amount": amount,
                            "amplitude": round((high - low) / prev_close * 100, 2) if prev_close > 0 else 0.0,
                        })
                    if sina_results:
                        logger.info(f"[Sina] 快速路径获取指数成功: {len(sina_results)} 条")
                        return sina_results
            except Exception as sina_e:
                logger.debug(f"[Sina] 快速路径失败, 回退 efinance: {sina_e}")
    ...
    def get_sector_rankings(self, n: int = 5) -> tuple[list[dict], list[dict]] | None:
        ...
        try:
            ...
            def _row_to_dict(row) -> dict:
                d = {"name": str(row[name_col]), "change_pct": float(row[change_col])}
                if amount_col and amount_col in df.columns:
                    try:
                        d["amount"] = float(row[amount_col])
                    except (ValueError, TypeError):
                        pass
                return d

            top_sectors = [_row_to_dict(row) for _, row in top.iterrows()]
            bottom_sectors = [_row_to_dict(row) for _, row in bottom.iterrows()]
