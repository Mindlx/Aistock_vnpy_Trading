...
class TestAgentSSECleanup(unittest.IsolatedAsyncioTestCase):
    ...
    async def _run_cleanup(self, fut_exception):
        ...
        try:
            ...
            agent_mod.logger.debug("agent executor cleanup timed out after 5s for session %s", "test-session")
        except Exception as exc:
            agent_mod.logger.warning("agent executor cleanup error (ignored): %s", exc, exc_info=True)
