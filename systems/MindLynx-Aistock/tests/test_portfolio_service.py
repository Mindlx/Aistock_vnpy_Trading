...
from src.services.portfolio_service import PortfolioConflictError, PortfolioOversellError, PortfolioService, _AvgState
...
class PortfolioServiceTestCase(unittest.TestCase):
        ...
        for market, currency, symbol, close, expected_symbol in [
            ...
            with self.subTest(market=market):
                ...
                position = self.service.get_portfolio_snapshot(
                    account_id=aid, as_of=date(2026, 1, 3), cost_method="fifo"
    ...
    def test_explicit_exchange_position_valuation_uses_exchange_qualified_symbol(self) -> None:
        account = self.service.create_account(
            name="Explicit Valuation", broker="Demo", market="cn", base_currency="CNY"
        )
        aid = account["id"]
        self.service.record_cash_ledger(
            account_id=aid,
    ...
    def test_backdated_trade_write_invalidates_future_cache(self) -> None:
        ...
        with self.db.get_session() as session:
            snapshot_count = (
                session.execute(select(PortfolioDailySnapshot).where(PortfolioDailySnapshot.account_id == aid))
                .scalars()
                .all()
            )
            position_count = (
                session.execute(select(PortfolioPosition).where(PortfolioPosition.account_id == aid)).scalars().all()
            )
            lot_count = (
                session.execute(select(PortfolioPositionLot).where(PortfolioPositionLot.account_id == aid))
                .scalars()
                .all()
            )
    ...
    def test_delete_trade_invalidates_cache_and_removes_source_event(self) -> None:
        ...
        with self.db.get_session() as session:
            trade_rows = session.execute(select(PortfolioTrade).where(PortfolioTrade.account_id == aid)).scalars().all()
            snapshot_rows = (
                session.execute(select(PortfolioDailySnapshot).where(PortfolioDailySnapshot.account_id == aid))
                .scalars()
                .all()
    ...
    def test_concurrent_sell_race_allows_only_one_write(self) -> None:
        ...
        threads = [threading.Thread(target=_worker, args=(f"sell-race-{idx}",), daemon=True) for idx in range(2)]
    ...
    def test_portfolio_write_session_maps_sqlite_locked_error(self) -> None:
        ...
        with (
            patch.object(self.db, "get_session", return_value=session),
            patch.object(
                session.connection(),
                "exec_driver_sql",
                side_effect=stmt_exc,
            ),
            self.assertRaises(PortfolioBusyError),
            repo.portfolio_write_session(),
        ):
            pass
