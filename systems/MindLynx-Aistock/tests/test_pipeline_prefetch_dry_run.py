...
class TestPipelinePrefetchBehavior(unittest.TestCase):
    ...
    def test_run_non_dry_run_prefetches_stock_names(self):
        ...
        pipeline.fetcher_manager.prefetch_stock_names.assert_called_once_with(["000001"], use_bulk=False)

    def test_run_dry_run_counts_existing_data_by_effective_trading_date(self):
        pipeline = self._build_pipeline(process_result=None)
        pipeline._resolve_resume_target_date = MagicMock(side_effect=[date(2026, 3, 27), date(2026, 3, 26)])
    ...
    def test_run_uses_one_frozen_reference_time_for_tasks_and_dry_run_stats(self):
        ...
        task_reference_times = [call.kwargs["current_time"] for call in pipeline.process_single_stock.call_args_list]
        stats_reference_times = [
            call.kwargs["current_time"] for call in pipeline._resolve_resume_target_date.call_args_list
        ]
