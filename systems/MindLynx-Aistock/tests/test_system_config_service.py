...
class SystemConfigServiceTestCase(unittest.TestCase):
    ...
    def test_validate_reports_ntfy_url_without_topic(self) -> None:
        validation = self.service.validate(items=[{"key": "NTFY_URL", "value": "https://ntfy.sh"}])

        self.assertFalse(validation["valid"])
        self.assertTrue(
            any(issue["key"] == "NTFY_URL" and issue["code"] == "invalid_ntfy_url" for issue in validation["issues"])
        )

    def test_validate_reports_gotify_url_with_message_endpoint(self) -> None:
        validation = self.service.validate(items=[{"key": "GOTIFY_URL", "value": "https://gotify.example/message"}])

        self.assertFalse(validation["valid"])
        self.assertTrue(
            any(
                issue["key"] == "GOTIFY_URL" and issue["code"] == "invalid_gotify_url" for issue in validation["issues"]
            )
        )
    ...
    def test_validate_reports_invalid_notification_route_channel(self) -> None:
        ...
        self.assertTrue(
            any(
                issue["key"] == "NOTIFICATION_REPORT_CHANNELS" and issue["code"] == "invalid_allowed_value"
                for issue in validation["issues"]
            )
        )

    def test_validate_reports_invalid_notification_quiet_hours(self) -> None:
        validation = self.service.validate(items=[{"key": "NOTIFICATION_QUIET_HOURS", "value": "9:00-18:00"}])

        self.assertFalse(validation["valid"])
        self.assertTrue(
            any(
                issue["key"] == "NOTIFICATION_QUIET_HOURS" and issue["code"] == "invalid_format"
                for issue in validation["issues"]
            )
        )

    def test_validate_reports_invalid_notification_timezone(self) -> None:
        validation = self.service.validate(items=[{"key": "NOTIFICATION_TIMEZONE", "value": "Mars/Olympus"}])

        self.assertFalse(validation["valid"])
        self.assertTrue(
            any(
                issue["key"] == "NOTIFICATION_TIMEZONE" and issue["code"] == "invalid_timezone"
                for issue in validation["issues"]
            )
        )

    def test_validate_reports_invalid_notification_min_severity(self) -> None:
        validation = self.service.validate(items=[{"key": "NOTIFICATION_MIN_SEVERITY", "value": "notice"}])

        self.assertFalse(validation["valid"])
        self.assertTrue(
            any(
                issue["key"] == "NOTIFICATION_MIN_SEVERITY" and issue["code"] == "invalid_enum"
                for issue in validation["issues"]
            )
        )
    ...
    def test_validate_reports_unknown_primary_model_for_channels(self) -> None:
        ...
        self.assertFalse(validation["valid"])
        self.assertTrue(
            any(issue["key"] == "LITELLM_MODEL" and issue["code"] == "unknown_model" for issue in validation["issues"])
        )
    ...
    def test_validate_reports_unknown_agent_primary_model_for_channels(self) -> None:
        ...
        self.assertTrue(
            any(
                issue["key"] == "AGENT_LITELLM_MODEL" and issue["code"] == "unknown_model"
                for issue in validation["issues"]
            )
        )
    ...
    @patch.object(
        ...
        "_parse_litellm_yaml",
        return_value=[
            {"model_name": "gemini/gemini-2.5-flash", "litellm_params": {"model": "gemini/gemini-2.5-flash"}}
        ],
    )
    ...
    def test_validate_reports_stale_primary_model_when_all_channels_disabled(self) -> None:
        ...
        self.assertTrue(
            any(
                issue["key"] == "LITELLM_MODEL" and issue["code"] == "missing_runtime_source"
                for issue in validation["issues"]
            )
        )
    ...
    def test_validate_accepts_minimax_model_as_direct_env_provider(self) -> None:
        ...
        self.assertFalse(
            any(
                issue.get("key") == "LITELLM_MODEL" and issue["code"] == "missing_runtime_source"
    ...
    def test_validate_reports_stale_agent_primary_model_when_all_channels_disabled(self) -> None:
        ...
        self.assertTrue(
            any(
                issue["key"] == "AGENT_LITELLM_MODEL" and issue["code"] == "missing_runtime_source"
                for issue in validation["issues"]
            )
        )
    ...
    def test_test_notification_channel_uses_temporary_items_without_persisting(self, mock_post) -> None:
        ...
        with self._notification_test_env():
            ...
            payload = self.service.test_notification_channel(
                channel="wechat",
                items=[
                    {"key": "WECHAT_WEBHOOK_URL", "value": "https://qyapi.example.com/cgi-bin/webhook/send?key=secret"}
                ],
                title="Test title",
                content="hello",
                timeout_seconds=3,
    ...
    def test_discover_llm_channel_models_classifies_error_scenarios(self, mock_get) -> None:
        ...
        quota_blocked_response = Mock(
            ok=False, status_code=403, text="account balance insufficient; your request was blocked"
        )
        quota_blocked_response.json.return_value = {
            "error": {"message": "account balance insufficient; your request was blocked"}
        }
        ...
        blocked_response.json.return_value = {
            "error": {"message": "Forbidden: your request was blocked by content policy"}
        }
    ...
    def test_update_warns_when_runtime_model_references_are_cleared(self) -> None:
        ...
        self.assertTrue(response["success"])
        warning = next(warning for warning in response["warnings"] if "已同步清理失效的运行时模型引用" in warning)
