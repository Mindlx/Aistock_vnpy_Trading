...
class TestAugmentHistoricalWithRealtime(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self._db_path = os.path.join(os.path.dirname(__file__), "..", "data", "test_issue234.db")
    ...
    @patch("src.core.pipeline_data.get_market_now")
    @patch("src.core.pipeline_data.is_market_open", return_value=True)
    @patch("src.core.pipeline_data.get_market_for_stock", return_value="cn")
    def test_appends_row_when_last_date_before_today(self, _mock_market, _mock_open, mock_now) -> None:
        today = date.today()
        ...
        mock_now.return_value = datetime(today.year, today.month, today.day, 10, 0, tzinfo=UTC)
    ...
    @patch("src.core.pipeline_data.is_market_open", return_value=True)
    @patch("src.core.pipeline_data.get_market_for_stock", return_value="cn")
    def test_updates_last_row_when_last_date_is_today(self, _mock_market, _mock_open, mock_now) -> None:
        today = date.today()
...
class TestEnhanceContextRealtimeOverride(unittest.TestCase):
    ...
    @patch("src.core.pipeline_data.get_market_now")
    @patch("src.core.pipeline_data.get_market_for_stock", return_value="cn")
    def test_today_overridden_when_realtime_and_trend_exist(self, _mock_market, mock_now) -> None:
        today = date.today()
        ...
        enhanced = self.pipeline._enhance_context(context, quote, None, trend, "贵州茅台")
    ...
    def test_today_not_overridden_when_trend_missing(self) -> None:
        context = {"code": "600519", "today": {"close": 15.0}}
        quote = _make_realtime_quote(price=15.72)
        enhanced = self.pipeline._enhance_context(context, quote, None, None, "贵州茅台")
