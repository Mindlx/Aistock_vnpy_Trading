...
class TestDataCleaning:
    ...
    def test_valid_cn_stock(self):
        """测试有效的 A股记录"""
        row = {"ts_code": "000001.SZ", "symbol": "000001", "name": "平安银行"}
    ...
    def test_valid_hk_stock(self):
        """测试有效的港股记录"""
        row = {"ts_code": "00700.HK", "name": "腾讯控股", "enname": "Tencent"}
    ...
    def test_valid_us_stock(self):
        """测试有效的美股记录"""
        row = {"ts_code": "AAPL", "name": "苹果", "enname": "Apple Inc."}
    ...
    def test_valid_us_stock_with_dot_suffix(self):
        ...
        row = {"ts_code": "BRK.B", "name": "", "enname": "BERKSHIRE HATHAWAY 'B'"}
    ...
    def test_us_dummy_filtered(self):
        """测试美股 DUMMY 记录被过滤"""
        row = {"ts_code": "DUMMY001", "name": "测试", "enname": "DUMMY Test Stock"}
        result = parse_stock_row(row, "US")
        assert result is None

    def test_us_dummy_case_insensitive(self):
        """测试 DUMMY 过滤不区分大小写"""
        row = {"ts_code": "DUMMY002", "name": "测试", "enname": "dummy test stock"}
        result = parse_stock_row(row, "US")
        assert result is None

    def test_empty_ts_code(self):
        """测试空 ts_code 被过滤"""
        row = {"ts_code": "", "symbol": "000001", "name": "平安银行"}
        result = parse_stock_row(row, "CN")
        assert result is None

    def test_empty_name(self):
        """测试空名称被过滤"""
        row = {"ts_code": "000001.SZ", "symbol": "000001", "name": ""}
        result = parse_stock_row(row, "CN")
        assert result is None

    def test_us_empty_enname(self):
        """测试美股空 enname 被过滤"""
        row = {"ts_code": "AAPL", "name": "苹果", "enname": ""}
        result = parse_stock_row(row, "US")
        assert result is None
...
class TestIntegration:
    ...
    def test_full_workflow_tushare(self, tmp_path):
        ...
        with open(hk_csv, "w", encoding="utf-8-sig", newline="") as f:
            ...
            writer.writeheader()
            writer.writerow({"ts_code": "00700.HK", "name": "腾讯控股", "enname": "Tencent"})

        us_csv = tmp_path / "stock_list_us.csv"
        with open(us_csv, "w", encoding="utf-8-sig", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["ts_code", "name", "enname"])
            writer.writeheader()
            writer.writerow({"ts_code": "AAPL", "name": "苹果", "enname": "Apple Inc."})
    ...
    def test_us_reused_symbols_are_deduplicated(self, tmp_path):
        ...
        us_csv = tmp_path / "stock_list_us.csv"
        with open(us_csv, "w", encoding="utf-8-sig", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["ts_code", "name", "enname", "list_date", "delist_date"])
            writer.writeheader()
            writer.writerow(
                {
                    "ts_code": "B",
