...
class _CriticalSectionTrackingNotifier:
    def __init__(self):
        ...
        self.generate_single_stock_report = MagicMock(side_effect=self._generate_single_stock_report)
...
class TestPipelineSingleNotifyThreadSafety(unittest.TestCase):
    def test_process_single_stock_serializes_direct_notification_path(self):
        ...
        threads = [
            threading.Thread(target=_worker, args=(code,), name=f"notify-{code}") for code in ("000001", "600519")
        ]
