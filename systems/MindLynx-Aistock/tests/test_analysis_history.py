...
class AnalysisHistoryTestCase(unittest.TestCase):
    ...
    def test_history_markdown_localizes_english_report_and_placeholder_name(self) -> None:
        ...
        with self.db.get_session() as session:
            row = (
                session.query(AnalysisHistory).filter(AnalysisHistory.query_id == "query_english_markdown_001").first()
            )
            if row is None:
                self.fail("未找到保存的历史记录")
            record_id = row.id
    ...
    def test_history_markdown_returns_persisted_market_review_report(self) -> None:
        ...
        with self.db.get_session() as session:
            row = session.query(AnalysisHistory).filter(AnalysisHistory.query_id == "market_review_query_001").first()
            if row is None:
                self.fail("未找到保存的历史记录")
            record_id = row.id
    ...
    def test_history_detail_returns_persisted_market_review_report(self) -> None:
        ...
        with self.db.get_session() as session:
            row = (
                ...
                .filter(AnalysisHistory.query_id == "market_review_query_detail_001")
                .first()
            )
            if row is None:
                self.fail("未找到保存的历史记录")
    ...
    def test_history_detail_localizes_english_summary_fields(self) -> None:
        ...
        with self.db.get_session() as session:
            row = session.query(AnalysisHistory).filter(AnalysisHistory.query_id == "query_english_detail_001").first()
            if row is None:
                self.fail("未找到保存的历史记录")
            record_id = row.id
    ...
    def test_history_markdown_uses_safe_bias_emoji_for_english_status(self) -> None:
        ...
        with self.db.get_session() as session:
            row = (
                session.query(AnalysisHistory)
                .filter(AnalysisHistory.query_id == "query_english_markdown_bias_001")
                .first()
            )
            if row is None:
                self.fail("未找到保存的历史记录")
