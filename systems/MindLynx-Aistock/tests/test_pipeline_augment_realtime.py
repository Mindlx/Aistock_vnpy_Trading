...
class AugmentRealtimeMarketDateTestCase(unittest.TestCase):
    ...
    @patch("src.core.pipeline_data.get_market_for_stock", return_value="us")
    def test_appends_virtual_row_with_market_local_date(self, _mock_market, mock_now, _mock_open):
    ...
    @patch("src.core.pipeline_data.get_market_now")
    @patch("src.core.pipeline_data.get_market_for_stock", return_value="us")
    def test_updates_existing_row_when_data_matches_market_date(self, _mock_market, mock_now, _mock_open):
    ...
    @patch("src.core.pipeline_data.is_market_open", return_value=False)
    @patch("src.core.pipeline_data.get_market_now")
    @patch("src.core.pipeline_data.get_market_for_stock", return_value="cn")
    def test_skips_augmentation_on_non_trading_day(self, _mock_market, mock_now, _mock_open):
        """Weekend/holiday: returns df unchanged."""
        mock_now.return_value = datetime(2026, 3, 28, 10, 0)

        df = _make_df([(date(2026, 3, 27), 30.0)])
        quote = SimpleNamespace(
            price=31.0, open_price=30.5, high=31.5, low=29.5, volume=100, amount=None, change_pct=1.0, pre_close=None
        )
