...
class TestNotificationServiceSendToMethods(unittest.TestCase):
    ...
    def test_send_isolates_channel_exceptions(self, mock_get_config: mock.MagicMock):
        ...
        with (
            mock.patch.object(service, "send_to_wechat", side_effect=RuntimeError("boom")),
    ...
    def test_send_with_results_reports_per_channel_attempts(self, mock_get_config: mock.MagicMock):
        ...
        with (
            mock.patch.object(service, "send_to_wechat", side_effect=RuntimeError("token=secret-token failed")),
    ...
    def test_send_to_context_is_not_limited_by_route(self, mock_get_config: mock.MagicMock):
        ...
        with (
            mock.patch.object(service, "send_to_context", return_value=True) as mock_context,
            mock.patch.object(service, "send_to_custom", return_value=True) as mock_custom,
        ):
            ok = service.send("content", route_type="report")
    ...
    def test_send_dedup_suppresses_static_channels_after_success(self, mock_get_config: mock.MagicMock):
        ...
        with mock.patch.object(service, "send_to_custom", return_value=True) as mock_custom:
            self.assertTrue(
                service.send("content at 12:00", route_type="report", dedup_key="report:aggregate:simple:600519")
            )
            self.assertFalse(
                service.send("content at 12:01", route_type="report", dedup_key="report:aggregate:simple:600519")
            )
    ...
    def test_noise_check_failure_does_not_block_static_send(self, mock_get_config: mock.MagicMock):
        ...
        with (
            mock.patch("src.notification_noise._evaluate_notification_noise", side_effect=RuntimeError("boom")),
            mock.patch.object(service, "send_to_custom", return_value=True) as mock_custom,
        ):
            ok = service.send("content", route_type="report")
...
class TestNotificationServiceReportGeneration(unittest.TestCase):
    ...
    @mock.patch("src.notification.get_config")
    def test_generate_dashboard_report_localizes_english_no_dashboard_fallback(self, mock_get_config: mock.MagicMock):
        mock_get_config.return_value = _make_config(report_renderer_enabled=False, report_language="en")
        service = NotificationService()
        result = AnalysisResult(
            code="AAPL",
    ...
    @mock.patch("src.notification.get_config")
    @mock.patch("requests.post")
    def test_send_to_gotify_via_notification_service(self, mock_post: mock.MagicMock, mock_get_config: mock.MagicMock):
    ...
    @mock.patch("src.notification.get_config")
    @mock.patch("requests.post")
    def test_send_to_ntfy_via_notification_service(self, mock_post: mock.MagicMock, mock_get_config: mock.MagicMock):
    ...
    @mock.patch("src.notification.get_config")
    def test_ntfy_url_with_unsupported_scheme_is_not_available(self, mock_get_config: mock.MagicMock):
    ...
    @mock.patch("src.notification.get_config")
    def test_send_to_context_delegates_to_source_context(self, mock_get_config: mock.MagicMock):
        """send_to_context 应委托给 _send_via_source_context 并返回其结果。"""
        mock_get_config.return_value = _make_config()

        service = NotificationService()
        with mock.patch.object(service, "_send_via_source_context", return_value=True) as mock_delegate:
            ok = service.send_to_context("test context content")

        self.assertTrue(ok)
        mock_delegate.assert_called_once_with("test context content")

    @mock.patch("src.notification.get_config")
    def test_send_to_context_returns_false_when_no_context(self, mock_get_config: mock.MagicMock):
        """无上下文渠道时 send_to_context 应返回 False。"""
        mock_get_config.return_value = _make_config()  # 无任何配置

        service = NotificationService()
        with mock.patch.object(service, "_send_via_source_context", return_value=False):
            ok = service.send_to_context("content")

        self.assertFalse(ok)

    @mock.patch("src.notification.get_config")
    def test_generate_daily_report_contains_stock_names_and_scores(self, mock_get_config: mock.MagicMock):
        """日报应包含每只股票的名称和评分。"""
        mock_get_config.return_value = _make_config()
        ...
        self.assertIn("宁德时代", report)
        self.assertIn("蓝天燃气", report)
        ...
        self.assertIn("核心看点", report)

        # 排序：高分在前
        self.assertLess(report.index("贵州茅台"), report.index("宁德时代"))
        self.assertLess(report.index("宁德时代"), report.index("蓝天燃气"))

    @mock.patch("src.notification.get_config")
    @mock.patch.object(NotificationService, "save_report_to_file")
    def test_send_daily_report_produces_report_and_sends(
        self, mock_save: mock.MagicMock, mock_get_config: mock.MagicMock
    ):
        """send_daily_report 应生成日报并调用 send 推送。"""
        cfg = _make_config(discord_webhook_url="https://discord.example/webhook")
        mock_get_config.return_value = cfg
        ...
        with mock.patch("requests.post", return_value=_make_response(204)):
            ok = send_daily_report(results)
