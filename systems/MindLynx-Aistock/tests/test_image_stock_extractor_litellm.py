...
class TestResolveVisionModel:
    ...
    def test_returns_empty_when_no_model_and_no_keys(self):
        cfg = _cfg(
            openai_vision_model=None, litellm_model="", gemini_api_keys=[], anthropic_api_keys=[], openai_api_keys=[]
        )
        with patch("src.services.image_stock_extractor.get_config", return_value=cfg):
...
class TestCallLitellmVision:
    ...
    def test_calls_litellm_with_image(self):
        ...
        with (
            ...
            patch(
                "src.services.image_stock_extractor.litellm.completion", return_value=self._good_response()
            ) as mock_comp,
        ):
    ...
    def test_raises_when_completion_returns_empty(self):
        ...
        with (
            patch("src.services.image_stock_extractor.get_config", return_value=cfg),
            patch("src.services.image_stock_extractor.litellm.completion", return_value=empty_resp),
            pytest.raises(ValueError, match="returned empty response"),
        ):
            _call_litellm_vision("b64", "image/jpeg")
...
class TestExtractStockCodesFromImage:
    ...
    def test_returns_items_and_raw(self):
        ...
        with (
            patch("src.services.image_stock_extractor.get_config", return_value=cfg),
            patch("src.services.image_stock_extractor.litellm.completion", return_value=self._good_vision_response()),
        ):
    ...
    def test_wraps_litellm_error_message(self):
        ...
        with (
            patch("src.services.image_stock_extractor.get_config", return_value=cfg),
            patch("src.services.image_stock_extractor.litellm.completion", side_effect=RuntimeError("network down")),
            pytest.raises(ValueError, match="Vision API 调用失败"),
        ):
