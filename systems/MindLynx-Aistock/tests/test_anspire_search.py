...
class TestAnspireSearchProvider(unittest.TestCase):
    ...
    def test_search_invalid_api_key(self, mock_requests):
        ...
        fake_response = _FakeResponse(status_code=401, json_data={"message": "Invalid API key"}, text="Unauthorized")
        ...
        # 错误消息可能因实现而异，这里做宽松检查
        self.assertTrue(
            "API" in response.error_message or "KEY" in response.error_message or "无效" in response.error_message
        )
    ...
    def test_search_empty_results(self, mock_requests):
        ...
        fake_response = _FakeResponse(status_code=200, json_data={"code": 200, "msg": "success", "results": []})
    ...
    def test_search_content_truncation(self, mock_requests):
        ...
        fake_response = _FakeResponse(
            ...
            json_data={
                "code": 200,
                "msg": "success",
                "results": [{"title": "长内容测试", "url": "https://example.com/long", "content": long_content}],
            },
        )
