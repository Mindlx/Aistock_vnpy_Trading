...
class AnalyzeRequest(BaseModel):
    ...
    stock_code: str | None = Field(None, description="单只股票代码", example="600519")
    stock_codes: list[str] | None = Field(
        None, description="多只股票代码（与 stock_code 二选一）", example=["600519", "000858"]
    )
    ...
    stock_name: str | None = Field(None, description="用户选中的股票名称（自动补全时提供）", example="贵州茅台")
    original_query: str | None = Field(None, description="用户原始输入（如茅台、gzmt、600519）", example="茅台")
...
class AnalysisResultResponse(BaseModel):
    ...
    report: Any | None = Field(None, description="分析报告")
...
class TaskStatus(BaseModel):
    ...
    progress: int | None = Field(None, description="进度百分比 (0-100)", ge=0, le=100)
    result: AnalysisResultResponse | None = Field(None, description="分析结果（仅在 completed 时存在）")
    market_review_report: str | None = Field(
        None,
        description="大盘复盘任务返回的报告文本（仅大盘复盘任务）",
    )
    error: str | None = Field(None, description="错误信息（仅在 failed 时存在）")
    stock_name: str | None = Field(None, description="股票名称")
    original_query: str | None = Field(None, description="用户原始输入")
    selection_source: str | None = Field(
        None,
        description="选择来源",
        pattern=SELECTION_SOURCE_PATTERN,
    )
    skills: list[str] | None = Field(None, description="本次任务使用的策略 skill ID 列表")
...
class TaskInfo(BaseModel):
    ...
    created_at: str = Field(..., description="创建时间")
    started_at: str | None = Field(None, description="开始执行时间")
...
class TaskListResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"total": 3, "pending": 1, "processing": 2, "tasks": []}}
